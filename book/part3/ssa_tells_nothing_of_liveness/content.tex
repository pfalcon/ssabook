...
This chapter illustrates the use of strict SSA properties\index{strict SSA form} to simplify and accelerate \emph{liveness analysis}\index{liveness}, which determines for all variables the set of program points where they are \emph{live}, i.e., their values are potentially used by subsequent operations.
Liveness information is essential to solve storage assignment problems, eliminate redundancies, and perform code motion.
For instance, optimizations like software pipelining, trace scheduling, register-sensitive redundancy elimination (see Chapter~\ref{chapter:pre_not_helped}), if-conversion (see Chapter~\ref{chapter:if_conversion}), as well as register allocation \ifregin (see Chapter~\ref{chapter:register_allocation})\fi heavily rely on liveness information.
...
These equations are usually solved by an iterative algorithm, propagating information backwards\index{backward analysis} through the control-flow graph (CFG) until a fixed point is reached and the liveness sets stabilize.
...
In this chapter, we show that, for strict SSA-form programs, the live-range of a variable has valuable properties that can be expressed in terms of loop nesting forest of the CFG and its corresponding directed acyclic graph, the \emph{forward-CFG}\index{forward control-flow graph}.
Informally speaking, and restricted to reducible CFGs\index{reducible CFG}, those properties for a variable $v$ are:
\begin{itemize}
\item
...
Irreducible CFGs\index{irreducible CFG} can be handled with a slight variation of the algorithm, with no need to modify the CFG itself.
...
Therefore, the live-range\index{live-range} of a variable can be computed using a backward traversal starting on its uses\index{upward exposed use} and stopping when reaching its definition (unique under SSA).
...
For a CFG node~$q$, representing an instruction or a basic block, a variable $\var{v}$ is \emph{live-in}\index{live-in} at~$q$ if there is a path, not containing the definition of $\var{v}$, from~$q$ to a node where $\var{v}$ is used (including $q$ itself).
It is \emph{live-out}\index{live-out} at~$q$ if it is live-in at some successor of~$q$.
...
Live-ranges of programs under strict SSA form exhibit certain useful properties (see Chapter~\ref{chapter:properties_and_flavours}), some of which can be exploited for register allocation \ifregin (see Chapter~\ref{chapter:register_allocation})\fi.
...
However, the semantics\index{liveness!\phifun}\index{\phifun, as multiplexer} of \phifuns\ (and in particular the actual place of \phiuses) should be defined carefully, especially when dealing with SSA destruction.
...
\begin{compactitem}
\item
	Its definition-operand\index{definitions, of \phifuns} is considered to be at the entry of $B_0$, in other words variable $a_0$ is live-in of $B_0$;
\item
	Its use-operands\index{uses, of \phifuns} are at the exit of the corresponding predecessor basic blocks, in other words, variable $a_i$ is live-out of basic block $B_i$.
\end{compactitem}
...
A well-known and frequently used approach to compute the live-in and live-out sets of basic blocks is backward data-flow analysis~\index{backward analysis} \ifconstantprop(see Chapter~\ref{chapter:constant_propagation_is_easier}, Section~\ref{novillo:sec:preliminaries})\fi.
The liveness sets are given by a set of equations that relate \emph{upward-exposed} uses\index{upward exposed use} and definitions to live-in and live-out sets.
...
The first version of the algorithm requires the CFG to be reducible~\index{reducible CFG}.
...
The key properties of live-ranges\index{live-in} under strict SSA form on a reducible CFG that 
...
\begin{algorithm}[H]
  ...
  \caption{Propagate live variables within loop bodies\index{loop header}.}
  \label{alg:loop_dfs}
\end{algorithm}
\medskip
...
forward CFG, $G_{\textit{fwd}}$\index{forward control flow graph}, obtained by removing all back-edges\index{back-edge} 
from the \@CFG $G$.
...
For the sake of clarity (and simplicity of implementation), we consider a representation where each loop has a unique entry node\index{entry node} as header.
...
We represent such as node as $\OLE{t}{s}$ (for ``Outermost Loop Excluding\index{outermost loop excluding}'').
...
An efficient implementation of \textit{OLE}\index{outermost excluding loop!OLE} is required.
...
Such a framework is well suited for tree-scan based register allocation\index{tree scan} \ifregin (see Chapter~\ref{chapter:register_allocation})\fi, SSA destruction\index{destruction, SSA} (see Chapter~\ref{chapter:alternative_ssa_destruction_algorithm}), or Hyperblock scheduling (see Chapter~\ref{chapter:machine_dependent_optimization_codegen}).
...
Queries in $O(1)$ are possible, at least for basic block boundaries, providing the use of sparsesets\index{sparseset} or bitsets\index{bitsets} to allow for efficient element-wise queries.
...
As explained in Section~\ref{sec:irreducible}, for irreducible CFG, the \emph{modified forward CFG}\index{foward control flow graph, modified} that redirects any edge $s\rightarrow t$ to the loop header of the outermost loop containing $t$ but excluding $s$ ($\OLE{t}{s}$), has to be used instead.
...
data-flow analysis, which goes back to Kildall~\cite{kildall73}. The algorithms are,
...
The technique of Gerlek et al.~can be considered as a precursor of the live variable analysis based on the Static Single Information (SSI) form conjectured by Singer~\cite{Singer06} and revisited by Boissinot et al.~\cite{BoissinotBDR12}.
...
Appel~\cite{appel97modern}. Instead of computing the liveness
...
Handling of irreducible CFG can be done through CFG transformations such as node splitting~\cite{JC97,Aho:1986:Book}.
