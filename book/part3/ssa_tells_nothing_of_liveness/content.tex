...
\item
	An actual query uses the def-use chain of the variable in question and determines the answer essentially by testing membership in precomputed sets of basic-blocks.
\end{compactenum}
...
The first pass propagates the liveness sets using a post-order traversal of the forward control-flow graph $\mathcal{F}_\mathcal{L}(G)$ of the CFG, obtained by removing all back-edges from the \@CFG.
...
There exist loop nesting forests representations with possibly multiple headers per irreducible loop.
...
average twice as fast as the ``single stack worklist'' approach.

Alternative ways to solve data-flow problems belong to the family of elimination-based algorithms~\cite{RyPa86b}. Through recursive reductions of the 
...
A more generalized analysis is given in~\cite{LivenessTACO}, both in terms of theoretical complexity and of practical evaluation (see Section~\ref{sec:path-explore} for such a path-exploration technique restricted to SSA programs).
...
The handling of any minimal loop nesting forest is detailed in~\cite{LivenessTACO}.
...
In our case the transformation does not have to maintain the semantic of the original program.
Ramalingam~\cite{Ramalingam02:Loops} proposed a transformation (without any exponential growth) that maintains dominance property.
Our transformation slightly differs from the one of Ramalingam as we want to keep the liveness information here, not only the dominance.
...
Liveness query can obviously be built on top of precomputed liveness sets. Queries in $O(1)$ are possible, providing the use of sparsesets~\cite{cooper:2004:engineering} or bitsets. Depending on the client for liveness analysis, \phifun should take different semantic. Detailed proofs are provided in~\cite{LivenessTACO} for different possible cases.
