...
% value is used for further propagation.  The simulations
...
is performed in order to find copy-related variables, followed by a rewrite
phase that eliminates spurious copies and renames variables.
...
are copies-of x$_1$, which in turn is a copy of itself.
...
\obacht{Flaw??}{The example seems to be flawed, e.g., x$_2$ should be defined
using a $\phi$ as block 0 has two predecessors, step 1 results in x$_4$ should
probably be x$_3$, assuming that x$_2$ might receive a value from outside of the
loop, the result x$_3 \rightarrow $ x$_8$ appears to be false.}
\begin{center}
