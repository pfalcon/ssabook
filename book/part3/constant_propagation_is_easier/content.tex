...
The \emph{propagation engine} presented in the following sections is an extension of the well known approach by Wegman and Zadeck for \emph{sparse conditional constant propagation} (also known as SSA-CCP). 
...
Data-flow analysis\index{data-flow analysis} is at the heart of many compiler transformations and optimizations, but also finds application in a broad spectrum of analysis and verification tasks in program analysis tools such as program checkers, profiling tools, and timing analysis tools. 
...
The analysis results are gathered from the input program by propagating information among its operations considering all potential execution paths. 
The propagation\index{propagation}\index{information!propagation} is typically performed iteratively until the computed results stabilize. 
Formally, a data-flow problem can be specified using a \emph{monotone framework} that consists of:
\begin{itemize}
  \item a \emph{complete lattice}\index{lattice}\index{data-flow analysis!lattice} representing the property space,
  \item a \emph{flow graph} resembling the control flow of the input program, 
        and
  \item a set of \emph{transfer functions}\index{transfer function}\index{data-flow analysis!transfer function} modeling the effect of individual
...
A particularly interesting class of partially ordered sets are \emph{complete lattices},\index{lattice!complete} where all subsets have a least upper bound as well as a greatest lower bound. 
...
\emph{least element} and the \emph{greatest element}, often denoted by $\bot$\index{$\bot$ (bottom, lattice)} and $\top$\index{$\top$ (top, lattice)} respectively.

An \emph{ascending chain}\index{lattice!chain}\index{lattice!ascending chain} is a totally ordered subset $\{l_1, \ldots, l_n \}$ of a complete lattice. 
...
Such analyses are termed \emph{backward analyses},\index{backward analysis}\index{data-flow analysis!backward} while those using the regular flow graph are \emph{forward analyses}.\index{forward analysis}\index{data-flow analysis!forward}

%///////////////////////////////////////////////////////////////////////////////
\textbf{Transfer Functions:}\index{data-flow analysis!transfer function}\index{transfer function}
...
Putting all those elements together---a complete lattice, a flow graph, and a set of transfer functions---yields an instance of a monotone framework\index{data-flow analysis!monotone}. 
...
A very popular and intuitive way to solve these equations is to compute the \emph{maximal (minimal) fixed point}\index{maximal fixed point} (MFP)\index{MFP|see{maximal fixed point}} using an iterative work list algorithm. 
...
A single flow edge can be appended several times to the work list in the course of the analysis. 
...
Besides the data dependencies, the SSA graph captures the \emph{relevant} join nodes\index{join node|exampleidx} of the CFG of the program. 
...
Even though the SSA graph captures data dependencies and the relevant join nodes in the CFG, it lacks information on other \emph{control dependencies}\index{dependence, control-}. 
...
Similar to monotone frameworks for traditional data-flow analysis, frameworks for \emph{sparse data-flow propagation}\index{data-flow, sparse} under SSA form can be defined using:
\begin{itemize}
...
The algorithm is shown in Algorithm~\ref{alg:constant_propagation_is_easier:propagation} and processes two work lists, the \emph{CFGWorkList}, which containing edges of the control-flow graph, and the \emph{SSA\-Work\-List}, which contains edges from the SSA graph. 
...
The \emph{CFGWorkList} is used to track edges of the CFG that were encountered to be \emph{executable}, i.e., where the data-flow analysis cannot rule out that a program execution traversing the edge exists. 
...
%% % Second, the semantics and placement of \phifuns.
%% %
%% This prohibits the
...
%% \phifuns are placed at join points with respect to the \emph{forward}
...
Even though data-flow analysis based on SSA graphs has its limitations, it is still a useful and effective solution for interesting problems, as we will show in the following example. 
Copy propagation\index{copy propagation} under SSA form is, in principle, very simple. 
Given the assignment $x \gets y$, all we need to do is to traverse the immediate uses of $x$ and replace them with $y$, thereby effectively eliminating the original copy operation. 
However, such an approach will not be able to propagate copies past \phifuns, particularly those in loops. 
A more powerful approach is to split copy propagation into two phases: 
Firstly, a data-flow analysis is performed to find copy-related variables throughout the program; 
...
\begin{figure}[t!]
 \def\mysp{\hspace*{1cm}}
  \begin{center}
    ...
    \subfloat[Copy relations on \phifuns]{
    \mysp
...
The analysis for copy propagation can be described as the problem of propagating the \emph{copy-of value}\index{copy-of value} of variables. 
...
The sparse propagation engine~\cite{novillo:bib:N05,WZ91}, as presented in the chapter, is based on the underlying properties of the SSA form. 
Other intermediate representations offer similar properties. 
\emph{Static Single Information} form (SSI) first introduced by Ananian~\cite{}Ananian99} and then later fixed by Singer~\cite{Singer06} has been designed with the objective of allowing both backward and forward problems.
It uses \sigmafuns, which are placed at program points where data-flow information for backward problems needs to be merged~\cite{novillo:bib:S04}.
But as illustrated by dead-code elimination (backward sparse data-flow analysis on SSA), the fact that \sigmafuns are necessary for backward propagation problems (or \phifuns for forward) is a misconsception.
This subttle point is further explained in Section~\ref{alg:ssi:propforward}.
Bod\'{\i}k also uses an extended SSA form, $e$-SSA, his goal being to eliminate array bounds checks~\cite{novillo:bib:BGV00}.
$e$-SSA is a simple extension that allows to also propagate information from conditionals. 
Chapter~\ref{chapter:ssi} revisits the concepts of static single information and proposes a generalization that subsumes all those extensions to SSA form.
Ruf~\cite{novillo:bib:R95} introduces the \emph{value dependence graph}, which captures both control and data dependencies. 
He derives a sparse representation of the input program, which is suited for data-flow analysis, using a set of transformations and simplifications.

The \emph{sparse evaluation graph} by Choi et~al.~\cite{Choi91} is based on the same basic idea as the approach presented in this chapter: 
intermediate steps are eliminated by by-passing irrelevant CFG nodes and merging the data-flow information only when necessary. 
Their approach is closely related to the placement of \phifuns and similarly, relies on the dominance frontier during construction. 
A similar approach, presented by Johnson and Pingali~\cite{novillo:bib:JO93}, is based on single-entry/single-exit regions. 
The resulting graph is usually less sparse, but is also less complex to compute. 
Ramalingam~\cite{Ramalingam02} further extends those ideas and introduces the \emph{compact evaluation graph}, which is constructed from the initial CFG using two basic transformations. 
The approach is superior to the sparse representations by Choi et~al. 
as well as the approach presented by Johnson and Pingali.
