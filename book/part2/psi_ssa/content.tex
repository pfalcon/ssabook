...
same \psiweb. The property of the $\psi$-C-SSA form is that the
renaming into a single variable of all variables that belong to the
same \psiweb, and the removal of the $\psi$ and $\phi$
...
\begin{itemize}
\item \emph{Psi-normalize}~: This phase will put all \psifuns
in what we call a {\em normalized} form.
\item \emph{Psi-web}~: This phase will grow \psiwebs from \psifuns, and will introduce repair code where needed.
\item \emph{Phi-web}~: This phase will extend the \psiwebs with
  \phifuns. This phase is very similar to the standard SSA destruction
  algorithm.
\end{itemize}
...
\begin{definition}[use points] Let ${a_0 = \psi(p_1?a_1, ..., p_i?a_i, ...,
...
interference graph is updated. This can be done conservatively, by letting ${a'_i}$  interfere with $a_i$ and with all the variables, except with those in the merged \psiweb, ${a_i}$ interferes with. The
conservative update of the interference graph may increase the number
of copies generated during the conversion to the $\psi$-C-SSA form.
...
\Begin{
 ...
 \ForEach{$a_i$ \textbf{in} $[a_n,\ a_{n-1},\ \dots, a_1]$}{
   ...
   \If{$\opndWeb \neq \psiWeb$}{
      \If{\textit{IGraph}.\textrm{interfere}(\psiWeb, \opndWeb)}{
         \Let{$a'_i$ be a freshly created variable} \;
         \Let{$C_i$ be a new predicated copy $p_i\cond a'_i\gets a_i$} \;
         append $C_i$ right after operation that defines $a_i$ \;
         replace $a_i$ by $a'_i$ in the \psifun \;
         $\opndWeb \gets \{a'_i\}$ \;
         \textit{IGraph}.addInterference($a'_i,\ a_i$) \;
         \ForEach{$v$ \textbf{in} $\textit{IGraph}.\textrm{interferenceSet}(a_i)$}{
            \lIf{$v \not\in\psiWeb$}{ \textit{IGraph}.addInterference($a'_i, v$) }\; 
         }
       }
...
At the beginning of the processing on the \psifun ${x =
  \psi(p?a,q?b,r?c)}$, all variables belong to their own \psiweb. The current \psiweb is set to the \psiweb of the
...
\psiweb now becomes ${\{x, b', c\}}$. Then variable \textit{a}
 is processed and inserted into the current \psiweb. The
...
When all \psifuns are processed, the \psiwebs built
from \psifuns are extended to include the variables in \phifuns. In this part, the algorithm presented in
chapter~\ref{chapter:alternative_ssa_destruction_algorithm} is used, with a few
modifications.

The first modification is that the \psiwebs must not be
initialized at the beginning of this process. They have already been
initialized at the beginning of the \psiweb step, and were extended
...
representation into a C-SSA form, are unchanged.

We have described a complete algorithm to convert a $\psi$-SSA
representation into a $\psi$-C-SSA representation. The final step to
convert the code into a non-SSA form is a simple renaming of all the
variables in the same \psiweb into a representative name. The
