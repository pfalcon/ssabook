...
Traditionally, non relational data-flow analyses bind information to pairs formed by a variable and a program point.
...
This data-flow analysis is an equation system that associates, with each program point $i$, an element of a lattice ${\cal L}$, given by the equation $x^i = \bigwedge_{s \in \textit{pred}(i)} F^s(x^s)$, where $x^i$ denotes the abstract state associated  with program point $i$, and $F^s$ is the transfer function associated with $s$, one of the control flow predecessors of $i$.
The analysis can be written\footnote{As far as we are concerned with finding its maximum solution. See for example Section~1.3.2 of~\cite{Nielson05}.} as a constraint system that binds to each program point $i$ and each $s\in \textit{pred}(i)$ the equation $x^i = x^i \wedge  F^s(x^s)$ or, equivalently, the inequation $x^i \sqsubseteq  F^s(x^s)$.
...
\begin{property}[SSI]
\label{pro:ssi}\textsc{Static Single Information:} Consider a forward (resp. backward) monotone PVL problem $E_{dense}$ stated as a set of constraints $[v]^i \sqsubseteq F_v^s([v_1]^s,\dots,[v_n]^s)$ for every variable $v$, each program point $i$, and each $s \in \textit{pred}(i)$ (resp. $s\in \textit{succ}(i)$).
...
Each node $i$ that has several predecessors (resp. successors), and for which $F_v^s(Y_{v_1}^s,\dots,Y_{v_n}^s)$ has different values on its incoming edges $(s,i)$ (resp. outgoing edges $(i,s)$) should have a \phifun at the entry of $i$ (resp. \sigmafun at the exit of $i$) for $v$
...
The assignment \[(v_1^1:l^1, \ldots, v_1^q:l^q) = \sigma(v_1) \ \parallel\  \dots \ \parallel\  (v_m^1:l^1, \ldots, v_m^q:l^q) = \sigma(v_m)\] represents $m$ \sigmafuns that assign to each variable $v_i^j$ the value in $v_i$ if control flows into block $l^j$.
...
Let us consider a unidirectional forward (resp. backward) PVL problem $E^{ssi}_{\textit{dense}}$ stated as a set of equations $[v]^i \sqsubseteq  F_v^s([v_1]^s, \dots, [v_n]^s)$ (or equivalently $[v]^i = [v]^i \wedge    F_v^s([v_1]^s, \dots, [v_n]^s$) for every variable $v$, each program point $i$, and each $s \in \textit{pred}(i)$ (resp. $s \in \textit{succ}(i)$). 
...
In other words, a \phifun such as $a=\phi(a_1:l^1,\dots,a_m:l^m)$, gives us $n$ constraints such as $[a]^i \sqsubseteq  F_v^{l^j}([v_1]^{l^j}, \dots, [v_n]^{l^j})$, which usually simplifies into $[a]^i \sqsubseteq [a_j]^{l^j}$. This last can be written equivalently into the classical meet $[a]^i \sqsubseteq \bigwedge_{l^j \in \textit{pred}(i)} [a_j]^{l^j}$ used in Chapter~\ref{chapter:constant_propagation_is_easier}.
Similarly, a \sigmafun $(l^1:a_1,\dots,l^m:a_m)=\sigma(a)$ at program point $i$ yields $n$ constraints such as $[a_j]^{l_j} \sqsubseteq  F_v^{l^j}([v_1]^{l^j}, \dots, [v_n]^{l^j})$, which usually simplifies into $[a_j]^{l_j} \sqsubseteq [a]^i$.
Given a program that fulfills the SSI property for $E^{ssi}_{\textit{dense}}$ and the set of transfer functions $F_v^s$, we show here how to build an equivalent sparse constrained system.  
...
characterized by the notion of join sets and iterated dominance frontier ($\textit{DF}^+$) seen in Chapter~\ref{chap:alternative_ssa_construction_algorithms}.
...
If $l$ is a branch point with $n$ successors that would contain a \sigmafun $(v_1:l^1, \ldots, v_n:l^n) =\sigma(v)$, then, for each successor $l^j$ of $l$, we insert at the beginning of $l^j$ an instruction $v_j = \phi(v:l)$.
...
\caption{(a) getting rid of copies and \sigmafuns; (b) implementing \sigmafuns via single arity \phifuns.}
\label{fig:sigImpl}
\end{figure}
...
We have already seen in Chapter~\ref{chap:classical_construction} how to replace \phifuns with actual assembly instructions; however, now we must also replace \sigmafuns and parallel copies.
