...
The liveness information can be computed for each \emph{individual} variable \emph{independently}; thus, it has the PVP property.
In other words, the liveness information for variable $v$ at program point $i$, e.g., $[v]^i$, can be expressed in term of its state at the successors $s$ of $i$,
...
Liveness analysis also has the PVL property.
...
The sum of these informations associated with all the variables at each program point can be written as a cross product ${\cal L}={\cal B}^n$.
