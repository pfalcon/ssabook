...
\caption{\label{fig:SSIfy} Split the live-ranges of $v$ to convert it to SSI 
form.}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \defall
    ...
    \begin{tabular}{rl}
      $_{21}$&\textrm{function stack.set\_use(instruction \var{inst})}:\\
      $_{22}$&\1\While $def(\textrm{stack.peek()})$ does not dominate inst: \textrm{stack.pop()}\\
      $_{23}$&\1$v_i = \textrm{stack.peek()}$\\
      $_{24}$&\1replace the uses of $v$ by $v_i$ in inst\\
      $_{25}$&\1\If $v_i\neq \bot$: set $\Uses(v_i)=\Uses(v_i) \cup$ inst
    \end{tabular}

    \begin{tabular}{rl}
      $_{26}$&\textrm{function stack.set\_def}(instruction inst):\\
      $_{27}$&\1let $v_i$ be a fresh version of $v$\\
      $_{28}$&\1replace the defs of $v$ by $v_i$ in inst\\
      $_{29}$&\1set $\Def(v_i)=$ inst\\
      $_{30}$&\1$\textrm{stack.push}(v_i)$
    \end{tabular}
    \caption{Versioning}
    \label{fig:Rename}
\end{figure}
