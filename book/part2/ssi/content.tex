...
information is a element in the dataflow lattice.
...
definition analysis.
The SSA form provides the static single information property to any
...
However, for dataflow analysis that derives information from the use sites of variables such as for the class inference analysis described in Section~\ref{sub:ssi:examples}, the information associated with $v$ might not be unique along its entire live-range: in that case the SSA form does not provide the SSI property.
...
$[v]^i$ by a single constraint variable $[v]$, for each variable $v$
and every $i\in \textrm{live}(v)$. 
Consider the problem of range analysis again. There are two types of program points that associate non-identity transfer functions: definitions and conditionals:
...
The other programs points can be ignored as we have the transfer function  $[v]^i = [v]^i \wedge F_v^s([v_1]^s, \dots, [v_n]^s)$ that simplifies into  $[v]^i = [v]^i$. 
...
The aim of this section is to introduce the $\phi$, $\sigma$, and parallel copy notations that we use to implement live-range splitting in our intermediate representation. Those are important elements of the propagation engine described in the section that follows. In short, a $sigma$-function (for a branch point) is the dual of a \phiop (for a join point), and a parallel copy is a copy that \emph{must} be done in parallel with another instruction.
...
The null pointer analysis example of Figure~\ref{fig:ssi:nullAnalysis}(d) that we will explain further uses the parallel copy notations for nodes $l_4$ and $l_6$.
...
A slightly modified version of this algorithm, seen in Figure~\ref{alg:ssi:propforward} propagates information forwardly. Of course, those algorithms can be made flow sensitive as for Algorithm~\ref{alg:constant_propagation_is_easier:ssa_propagation}. 
...
Figure~\ref{fig:rangeAnalysis-sparse}(b) shows the result to the range analysis problem ran on this form.
We can see that this result is an assignment of each variable to a unique range interval.
...
%TBI\section{Construction and Destruction of the Intermediate Program Representation}
\label{sec:building}
\def\Sdown{\downarrow}
\def\Sup{\uparrow}
\def\SS{{\cal P}}
...
%TBI\subsection{Splitting strategy}
...
%TBI\subsection{Splitting live ranges}
...
%TBI\subsection{Variable Renaming}
...
%TBI\subsection{Dead and Undefined Code Elimination}
...
%TBI\subsection{Implementing parallel copies, $\phi$ and $\sigma$-functions and Destructing SSI}
\label{sub:special}
