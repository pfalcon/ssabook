...
Because this approach keeps information at each \progpoint, we call it {\em 
  dense},\index{dense analysis} in contrast with the sparse analyses
\ifconstantprop
seen in 
...
\begin{figure}[htbp]
  ...
  \begin{algorithm}[H]
    ...
    \Comment{compute the set of split nodes}
      ...
      $S \gets \SS_v \cup \Sup \cup \Sdown$\;\nllabel{ssi:split:start-insert}
       \Comment{Split live-range of $v$ by inserting $\phi$, $\sigma$, and 
       copies}
       \ForEach{$i \in S$}{
...
\begin{figure}[htbp]
  ...
  \begin{algorithm}[H]
    \Fn{\ssiRename{var \var{v}}}{
      \Comment{Compute use-def \& def-use chains.}
  ...
  \begin{algorithm}[H]
    \Fn{\ssiStackSetUse{instruction \var{inst}}}{
      \Comment{We consider here that $\texttt{stack.peek()}=\bot$ if
...
Just\index{dead code elimination}\index{undefined code elimination} as Algorithm~\ref{alg:classical_construction_algorithm:pruning}, the algorithm in Figure~\ref{fig:clean} eliminates \phifuns and parallel copies that define variables not actually used in the code. By symmetry, it also eliminates \sigmafuns  and parallel copies  that use variables not actually defined in the code.
...
\hspace{-1.3cm}
  \subfloat[]{
    ...
    \ifx\dousetikz\undefinedmacro
      \missingfigure{Figure does note compile in overleaf!!!}
    \else
...
Traditional\index{SSI destruction} instruction sets do not provide \phifuns nor \sigmafuns.\index{SSI destruction}
