...
As in the index propagation analysis in Section~\ref{alg},
the worst-case execution-time complexity for dead store elimination
is $O(S \times k^2)$, where $S$ is the size of the input method and
$k$ is an upper bound on the number of defs and uses for a single
heap array.
...
\REM{
An upper bound on
the size of $\U^A_{ind}$ equals the number of distinct index value numbers
present in non-$\phi$
definitions of array $A$ in the program, which will be no
greater than (and usually much smaller than) the number of non-$\phi$
definitions of array $A$.
However, if $\U^A_{ind}$ and the $\DEAD$ sets become prohibitively large
for some input program, we always have the option of dropping elements
from $\DEAD$ sets for the sake of efficiency (at the cost
of precision), while still guaranteeing
correctness.
}
\REM{
{\it Additional details on the store elimination
...
\REM{
{\it Further details of the store elimination algorithm have been omitted
due to space limitations.}


{\it TODO or to remove: We conclude this section with a brief description of how our store elimination
algorithm will process the example program in figure~\ref{fig:ex2}(c).
The Array SSA form for this example 
is shown in figure~\ref{fig:A-SSA-form}
}
...
$\KILL(A_3) = \{\; \V(j) \;\}$\\
\\
$\KILL(A_5) = \{\; \V(i) \;\}$ {\it shouldn't the be \V(k)?} \\
...
\hline
\end{tabular}
\caption{Trace of store elimination algorithm for program in figure \protect{\ref{fig:ex2}}(c)}
\label{fig:A-SSA-form}
\end{figure}
}
