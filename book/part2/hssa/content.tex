...
Aliasing\index{\mufun}\index{\chifun} occurs inside a compilation unit when a single one single storage location (that contains a value) can be potentially accessed through different program "variables".
...
Intuitively, the solution to this problem is to factor all variable versions\index{variable version} that are considered \emph{useless} together, so that no space is wasted to distinguish among them. We assign number 0 to this special variable version, and call it "{\em zero version}"\index{zero version}.
...
Cite the original paper from Chow \cite{hssa}.
