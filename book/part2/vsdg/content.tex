...
Given that a program is represented as an SSA graph, the task of finding induction variables is simplified. A \texttt{basic induction variable} is a variable that appears only in the form:
...
where \texttt{k} is a constant or loop invariant. Wolfe made the observation that each basic linear induction variable will belong to a nontrivial SCR in the SSA graph. Each such SCR must conform to the following constraints:
...
Statement nodes represent instructions in the program. Predicate nodes test a conditional statement and have {\sf true} and {\sf false} edges to represent the choice taken on evaluation of the predicate. Region nodes group all nodes with the same control dependencies together, and order them into a hierarchy. If the control dependence for a region node is satisfied, then it follows that all of its children can be executed. Thus, if a region node has three different control-independent statements as immediate children, then these could potentially be executed in parallel. Diagrammatically, rectangular nodes represent statements, diamond nodes predicates, and circular nodes are region nodes. Filled edges represent data dependence, and dashed edges represent control dependence. Loops in the PDG are represented by back edges in the control dependence subgraph. We show example code translated into a PDG in Figure~\ref{fig:pdg}. 
...
Next, region nodes are added to the PDG. Each region node summarizes the set of control conditions and groups all nodes with the same set of control conditions together. Region nodes are also inserted so that predicate nodes will only have two successors, thus hierarchically ordering the control dependences. To begin with, an unpruned PDG is created by checking, for each node of the CFG, which control region or control dependence node it belongs to. This is done by traversing the postdominator tree in postorder, and using a hash table to map sets of control dependences to region nodes. For each node $N$ visited in the postdominator tree, the hash table is checked for an existing region node with the same set $CD$ of control dependences. If none exists, a new region node $R$ is created with these control dependences and entered into the hash table. $R$ is made to be the only control dependence predecessor of $N$. Next, the intersection $INT$ of $CD$ is computed for each immediate child of $N$ in the postdominator tree. If $INT=CD$ then the corresponding dependences are deleted from the child and replaced with a single dependence on the child's control predecessor. Then, a pass over the graph is made to make sure that each predicate node has a unique successor for each truth value. If more than one exists, a region node is created and inserted as the successor of that particular predicate node's $T$ or $F$ edge.
...
The structure of the PDG allows for parallelism to be detected easily. In order to discover potential parallelism from the CFG, the if-conversion transformation must first be performed. However, the PDG can \textit{directly} be used to detect parallelism. Any node in the PDG that is not contained in a strongly connected region consisting of \textit{both} control and data dependences can be parallelised. In the example in Figure~\ref{fig:pdgparallel} we have a program containing a loop consisting of multiple assignment statements, and the corresponding PDG. Here, since the statements \texttt{d = e + f} and \texttt{g = h + i} (outlined in bold) in the loop do not form a strongly connected region, so they are parallelisable. The statements \texttt{a = b + c} and \texttt{i = i + a} are not, since the assignment to \texttt{i} is data dependent on the assignment to \texttt{a}.
...
\caption{Detecting parallelism with the PDG. The statements \texttt{d = e + f} and \texttt{g = h + i} (outlined in bold) are parallelisable.}
...
If forwards substitution were to be used in order to determine whether the assertion is correct, then the symbolic value of \texttt{J} must be discovered, starting at the top of the program in statement \texttt{R}. Forward propagation through this program results in statement \texttt{T} being $\mathtt{assert(}(if $ $\mathtt{P}$ $then$ $\mathtt{Expr-1}$ $else$ $\mathtt{Expr)}$ $\leq$ $\mathtt{Expr}$  $\mathtt{)}$, thus the \texttt{assert} statement evaluates to true. In real, non-trivial programs, these expressions could get unnecessarily long and complicated. 

Using GSA allows for backwards, demand-driven substitution. The program above has the following GSA form:
...
Using this backwards substitution technique, we start at statement \texttt{T}, and follow the SSA links of the variables. This allows for skipping of any intermediate statements that do not affect variables in \texttt{T}. Thus the substitution steps are:
...
The backwards substitution then stops because enough information has been found, avoiding the redundant subtitution of $\mathtt{JMAX_1}$ by $\mathtt{Expr}$. In non-trivial programs this can greatly reduce the number of redundant substitutions, making symbolic analysis significantly cheaper.

\section{Value State Dependence Graph}

The gating functions defined in the previous section were used in the development of a sparse data flow graph IR called the Value State Dependence Graph (VSDG). The Value State Dependence Graph is a directed graph consisting of operation
...
this way. Here, we also see how two syntactically different programs can map to the same structure in the VSDG.
...
% TODO: references for worked example papers.

Gating functions can be used to create directly interpretable $\phi$-functions. These are used in Gated Single Assignment Form \cite{93578,207115,Havlak93constructionof}.
