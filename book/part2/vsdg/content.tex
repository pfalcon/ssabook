...
% TODO: replace this para
%Assuming that the CFG and SSA have already been constructed, then building GSA proceeds in two steps. Firstly, replacing $\phi$-functions with directed acyclic graphs of $\gamma$ functions, and secondly, building the controlling predicate of loops for use in its $\eta$ functions. The dominator tree is built for a procedure, and for each loop a preheader node and postbody node are added. Wherever the sink of a loop-exit edge has other in-edges, the loop-exit edge is split with the addition of a postexit node.$\mu$ functions are then placed at loop headers instead of $\phi$ functions. Every $\mu$ node added represents a variable modified in the loop, and placeholder $\eta$ functions are placed for the same variable at the postexit node for the loop. These hold the place for later creation of $\eta$ functions with predicates, and ensure proper placement of $\phi$ assignments for merges of values from different exits. Next, the remaining $\phi$ functions are converted to DAGs of $\gamma$ functions, and then other $\gamma$ DAGs are built to represent loop-termination conditions. References to detailed algorithms for the construction of GSA can be found in the chapter summary. 
