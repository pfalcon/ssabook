...
One of the seminal graph representations is the Control-Flow Graph\index{control-flow graph} (CFG), which was introduced to explicitly represent possible control paths in a program. 
...
The data-flow graph\index{data-flow graph|mainidx} (DFG) is a directed graph $G=(V,E)$ where the edges $E$ represent the flow of data from the result of one operation to the input of another. 
...
This graph is therefore a \textit{demand-based} representation\index{SSA graph, demand-based}. 
...
Reversing the direction of each edge would provide the widely used \emph{data-based}\index{SSA graph, data-based} representation. 
The SSA Graph can be constructed from a program in SSA form by \textit{explicitly} adding use-def chains\index{use-def chains}. 
...
The PDG was developed to aid optimizations requiring reordering of instructions and graph rewriting for parallelism\index{parallelization}, as the strict ordering of the CFG is relaxed and complemented by the presence of data dependence information. 
...
Predicate nodes\index{PDG, predicate nodes} test a conditional statement and have {\sf true} and {\sf false} edges to represent the choice taken on evaluation of the predicate. 
...
Dashed/full edges respectively represent control/data dependencies.}
\label{fig:pdg}
\end{figure}
...
First, control dependence\index{control dependence} between statements and predicate nodes are computed; 
...
A statement $B$ that is to be executed after a statement $A$ in the original sequential ordering of the program (data)-depends\index{data dependence} on $A$ in the following situations: 
...
\item The $\phientry$\index{$\phientry$-function} function is inserted at loop headers to select the initial and loop carried values. A $\phientry$ function of the form $\phientry(v_{init},v_{iter})$, has $v_{init}$ as the initial input value for the loop, and $v_{iter}$ as the iterative input. We replace \phifuns at loop headers with $\phientry$ functions.
