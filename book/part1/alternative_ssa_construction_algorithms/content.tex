...
We first present some properties of the \DJ-graph\index{DJ-graph@\DJ-graph} that allow to compute the iterated dominance frontier of some set of nodes $S$ by traversing the \DJ-graph from leafs to root. 
...
Basic algorithm for the placement of \phifuns consists of computing the iterated dominance frontier (\iDF) for a set of all definition points (or nodes where variables are defined). 
Let $\Defs{v}$ be the set of nodes where variable $v$ is defined. 
Given that the dominance frontier for a set of nodes is just the union of the \DF set of each node, we can compute $\iDF(\Defs{v})$ as a limit of the following recurrence equation (where $S$ is initially $\Defs{v}$):
\begin{eqnarray*}
\iDF_1(S) &=& \DF(S) \\
...
We now propose a linear time algorithm for computing, for a given set of nodes $S$, the $\iDF(S)$ set without the need for explicitly pre-computing the full \DF set. 
The proposed algorithm uses the \DJ-graph (See Section~\ref{sec:classical_construction_algorithm:phiinsertion} and Figure~\ref{fig:classical_construction_algorithm:iDF-J}) representation of a CFG. 
The \DJ-graph for our example CFG is also shown in Figure~\ref{fig:cfg}(b). 
...
Consider the \DJ-graph shown in Figure~\ref{fig:cfg}(b) where the depth of a node is the  distance from the root in the dominator tree.
The first key observation is that a \DF-edge never goes down to a higher depth.
To give a row intuition, suppose there is a \DF-edge from 8 to 7, this would mean there exists a path from 3 to 7 through 8 without flowing through 6:
This would contradict the dominance of 7 by 6. 
...
To illustrate the key observation consider the example \DJ-graph in Figure~\ref{fig:cfg}(b), and let us compute $\iDF(\{3,8\})$. 
...
\begin{algorithm}
  ...
  \While(\tcc*[f]{Get node from the deepest depth}){$x \gets \getN$}{  \label{C:get} 
    $\textit{current\_x} \gets x$\;
    $x.\visited \gets \true$\;
    Visit($x$)\tcc*[r]{Find $\DF(x)$}
  }
  \textbf{return} \iDF
\end{algorithm}
...
\begin{procedure}
  ...
  \ForEach{J-edge $y \st{J} z$}{
    \If{$z.\depth \leq \textit{current\_x}.\depth$}{
      \If{$z \not \in \iDF$}{
        $\iDF \gets \iDF \cup \{z\}$\;   \label{C:idf}
        \lIf{$z \not  \in S$}{
...
Whenever it notices that the depth number of the destination node of a J-edge is less than or equal to the depth number of the $\textit{current\_x}$, the destination node is added to the $\iDF$ set (Line~\ref{C:idf}) if it is not present in \iDF already. 
...
the $\iDF$ set is updated to hold node~2 according to Line~\ref{C:idf} of the Visit procedure. 
...
The first and direct variant of the approach laid out above is poetically termed TDMSC-I. 
This variant works by scanning the \DJ-graph in a top-down fashion as shown in Line~\ref{C:bfs} of Figure~\ref{F:tdmsc}. 
All $\iDF(x)$ sets are set to the empty set before the initial pass of TDMSC-I. 
...
During this process, for each node $z$ encountered, if there is an \emph{incoming} J-edge $y \st{J} z$ as in Line~\ref{C:jedge}, then a separate bottom-up pass starts at Line~\ref{C:mwhiles}. 
...
Line~\ref{C:lnode} is used for the inconsistency check. 
...
Line~\ref{C:lnode} of the algorithm visits incoming edges to \textit{lx} only when \textit{lx} is at the same depth as $z$, which is the current depth of inspection and the incoming edges to \textit{lx}'s posterity are at a depth greater than that of node $z$ and are unvisited yet.
...
Moving top-down over the graph, the first J-edge encountered is $7 \st{J} 2$. 
