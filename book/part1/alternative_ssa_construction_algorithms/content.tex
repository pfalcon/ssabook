    ...
    \ForEach{incoming edge $e = y \st{J} z$}{ \label{line:alt:jedge}
      \If{not $e.\visited$}{
        ...
        \While{$(x.\depth\ge z.\depth)$}{
          ...
          $\textit{lx} \gets x$\;
          $x \gets \textrm{parent}(x)$ \tcc*{dominator tree parent}
        }
        \ForEach{incoming edge $e' = y^{'} \st{J} \textit{lx}$}{
          \label{line:alt:lnode}
          \If{$e'.\visited$}{
            \If(\tcc*[f]{Check inconsistency}){$\iDF(y') \not\supseteq 
              \iDF(\textit{lx})$}{%
...
The loop with header node $2$ contains the loop with header node $9$.
    ...
    \hfill
    \subfloat[CFG]{\tikzfigure{lnfcfg}}
    \hfill
    % \qquad
    \subfloat[Loop Nesting Forest]{\tikzfigure{lnfred}}
    \hfill\null
