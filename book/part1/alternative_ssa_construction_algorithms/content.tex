...
\x2    InsertNode($x$);
\x2    $x.\textit{visited} = \false $;
\x1 {\bf end for}
\x1 {\bf while} (($z = \textit{GetDeepestNode}()) \neq \nullprog$)  \label{C:get} // Get node from the deepest level
\x2   $\textit{currentRoot} = z$;
\x2  $z.\textit{visited} = \true$ ;
...
\{
\begin{code}
  \x1 {\bf foreach} (node $y \in \textit{Succ}(x)$) {\bf do}
\x2  {\bf if} ($x \st{} y$ is a  J edge) {\bf then}
\x3   {\bf if} ($y.\level \leq \textit{currentRoot}.\level$) {\bf then}
...
\x3   {\bf if} ($y.\textit{visited} == \false $) {\bf then}
\x4    $y.\textit{visited} = \true$;
\x4    // if($y.\textit{boundary} == \false$)   \label{C:cached} // See the section on Further Reading for details
