...
The  placement  of $\phi$-functions is an important step in the construction of Static Single Assignment (SSA) form \cite{cfr}.
...
placing \p-functions using the DJ(Dominator-Join) graph representation of a CFG \cite{sreedhar_popl}.
Then we will discuss the notions of merge set and merge relation, and present Das and Ramakrishna's algorithm for placing \p-functions based on merge relation and using DJ graph \cite{das}. Finally
we describe another approach for placing \p-functions based on the loop nesting forest proposed by Ramalingam \cite{rama}.
...
Sreedhar and Gao proposed the first linear time algorithm for computing the IDF set without the need for explicitly computing the full DF set \cite{sreedhar_popl,sreedharthesis}. Sreedhar and Gao's original algorithm was implemented using the DJ graph representation of a CFG. A DJ graph
...
\begin{center}
$DF(x) = \{y | \exists z \in SubTree(x) \; \wedge \; z \st{J} y \; \wedge \; y.level \leq x.level \}$
\end{center}
...
Now we can extend the above idea to compute the IDF for a set of nodes, and hence the placement of phi. This algorithm does not precompute DF; given a set of initial nodes $N_{\alpha}$ for which we want to compute the relevant set of $\phi$-functions,
...
Thus, to compute $DF(x)$ where $x$ is an ancestor of $y$ in the DJ graph one need not compute from scratch but can re-use the information computed as part of DF(y) as shown. For this, we need to compute DF of a deeper node($y$) (based on level) before computing the DF of a shallower node ($x$).

\begin{flalign*}
DF(x) & = \{w|w \in DF(y) \wedge w.level \leq x.level\} \cup \\
          &  \{w|t \in Subtree(x) \setminus Subtree(y) \wedge t \st{J} w \wedge w.level \leq x.level \}
%}. 
\end{flalign*}
...
irreducible loops. The algorithm is based on Ramalingam's work \cite{rama}.
...
\end{itemize}  
  ...
  \end{figure}
    ...
    We will walk through some of the steps of this algorithm for computing $IDF(\{4,5,7,12\})$. We will start at vertex 4. For this vertex, ${\it predecessors} = \{3\}$. But as $3 \notin IDF \cup X \cup \{entry\}$, ${\it ReachingDefs} = {\it UniqueReachingDefs}(3)$ according to Line ~\ref{C:urdf}. Hence, 
${\it ReachingDefs} = \{entry\}$. As $\|{\it ReachingDefs}\|$ equals 1, ${\it UniqueReachingDefs}(4) = \{entry\}$. The same logic applies for vertices 5 and 7, and so,  ${\it UniqueReachingDefs}(5) = {\it UniqueReachingDefs}(7) = \{entry\}$.
