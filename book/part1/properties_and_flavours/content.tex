...
\chapter{Properties and flavours \Author{P.~Brisk \andAuthor F.~Rastello}}
...
Under SSA form, each variable is defined once. Def-use chains\index{def-use chains} are data structures that provide for the single definition of a variable the set of all its uses. In turn, a use-def chain\index{use-def chain}, which under SSA consists of a single name, uniquely specifies the definition that reaches the use. As we will illustrate further in the book (see~Chapter~\ref{chapter:constant_propagation_is_easier}) def-use chains are useful for forward data-flow analysis as they provide direct connections that shorten the propagation distance between nodes that generate and use data-flow information. 
...
a \emph{join node}\index{join node} of $n_{1}$ and $n_{2}$ if there exist at least two
...
A procedure is defined to be \emph{strict}\index{strictness} if every variable
is defined before it is used along every path from the entry
to exit point; otherwise, it is \emph{non-strict}\index{non-strict}.
...
\emph{dominance property}\index{dominance}: each use of a variable is dominated by
its definition.
...
The immediate dominator\index{dominator!immediate}\index{immediate|see{dominator}} or ``idom'' of a node $N$ is the unique node that strictly dominates $N$ but does not strictly dominate any other node that strictly dominates $N$. All nodes but the entry node have immediate dominators. A dominator tree\index{dominator!tree} is a tree where the children of each node are those nodes it immediately dominates. Because the immediate dominator is unique, it is a tree with the entry node as root. 
...
This property allows also efficient algorithms to test whether two variables \emph{interfere}\index{interference} (see Chapter~\ref{chapter:alternative_ssa_destruction_algorithm}):
...
an important role in register allocation\index{register allocation}, as the register assignment problem can be expressed as a coloring problem of the interference graph\index{interference graph}.
...
a traversal of the dominator tree, i.e., a ``tree scan,''\index{tree scan} can color all of
...
definition, as opposed to all program points. Pruned SSA\index{pruned SSA}\index{SSA!pruned} form satisfies these properties. 
...
program that has been converted to SSA form into $\phi$-equivalence classes that we will refer as \phiwebs.\index{\phiweb}\index{web|see{\phiweb}}
...
Conventional SSA form (C-SSA)\index{SSA!conventional}\index{conventional|see{SSA}}\index{C-SSA}
...
to a non-conventional\index{SSA!transformed}\index{tranformed|see{SSA}}\index{T-SSA} (T-SSA for Transformed-SSA) form, in which some variables belonging to
...
for correctness, this is a fairly restrictive definition of interference,\index{interference} based on static considerations. Consider for instance the case when two simultaneously live variables contain in fact the same value, then it would not be a problem to put both of them in the same register.
...
The notion of pruned SSA has been introduced by Choi, Cytron, and Ferrante~\cite{ChoiJan91}. The example of Figure~\ref{fig:properties_and_flavors:pruned} to illustrate the difference between pruned and non pruned SSA has been borrowed from Cytron et al.~\cite{CytronOct91}. 
The notions of conventional and transformed SSA were introduced by Sreedhar et al. in their seminal paper~\cite{Sreedhar:1999:SAS} for destructing SSA form.
