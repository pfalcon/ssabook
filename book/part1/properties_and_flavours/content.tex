...
Second, as it is easy to associate to each variable its single defining operation, use-def chains can be explicitly represented and maintained almost for free. 
...
A procedure is defined to be \emph{strict}\index{strictness} if every variable is defined before it is used along every path from the entry to exit point; 
...
The notion of pruned SSA has been introduced by Choi, Cytron, and Ferrante~\cite{ChoiJan91}. 
The example of Figure~\ref{fig:properties_and_flavors:pruned} to illustrate the difference between pruned and non pruned SSA has been borrowed from Cytron et al.~\cite{CytronOct91}. 
