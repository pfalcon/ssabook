...
The conversion to minimal SSA form replaces each variable $v$ in the pre-SSA
...
\end{enumerate}
Therefore, this notion of $\phi$-relationship is itself an equivalence
...
We refer to an equivalence class as an SSA-web. 
Let $\web{x}$ denote the SSA-web containing variable $x$. 

Under conventional SSA Form, each SSA-web
...
SSA destruction
starting from non-conventional SSA Form requires a conversion to 
...
to a non-conventional SSA Form, in which some variables belonging to
