...
conditionally executed block following an \texttt{if} statement beginning:
\begin{ccode}
if (x==0)
\end{ccode}
however the
...
they are sequentialized using conventional %%parallel 
copy operations,
% at the end of predecessor control flow nodes.
as described in Chapters~\ref{chapter:classical_construction_algorithm} and~\ref{chapter:alternative_ssa_destruction_algorithm}.
% (forward note to Fabrice's section on this,
% in later chapter).

%{\emph{JS TODO} - move discussion of parallel copies to from ch3 to
%  ch1. You mention in the vanilla chapter the fact that phi are to be
%  executed in parallel. An option is to explain the parallel semantics
%  in the vanilla chapter using parallel copies on incoming edges.

In several places throughout the book,
we will adopt a more expressive syntax for
$\phi$-functions, associating 
predecessor basic block labels $B_i$
with corresponding SSA variable names $a_i$, i.e.\
$a_0 = \phi(B_1:a_1, \ldots, B_n:a_n)$.
This will be particularly useful when we introduce
SSA construction algorithms in Chapter 3 (ref).
...
such as $\gamma$-functions (see Chapter~\ref{chapter:vsdg}), which take
...
It is important to note that static single assignment 
does not prevent multiple assignments to a variable
...
\begin{center}
  ...
  \label{sub:vanilla:cfg}
  }
%
  \subfloat[]{
  \includegraphics[scale=0.3]{ssa_example.png}
  \label{sub:vanilla:ssa-ex}
  }
\end{center}
