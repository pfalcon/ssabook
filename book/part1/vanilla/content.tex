...
\begin{center}
\subfloat[dense]{
  \includegraphics[scale=0.9]{zero-1.pdf}
}
\hfill
\subfloat[SSA based]{
  \includegraphics[scale=0.9]{zero-2.pdf}
}
%\tikzsubfigures{zero}
...
~\\
\textbf{Current Usage }
The majority of current commercial and open-source compilers, including  GCC, Open64, Sun's~HotSpot~JVM, IBM's~Java~Jikes~RVM, Chromium~V8 JavaScript~engine, Mono, or~LLVM,
...
Initially developed to facilitate the development of high-level program transformations, SSA form has gained much interest due to its favorable properties that often allow to simplify algorithms and reduce computational complexity. 
...
Recent research on register allocation\index{register allocation} (see Chapter~\ref{chapter:register_allocation}) even allows to retain SSA form until the very end of the code generation process.
...
This serves to simplify the structure of variable \emph{def-use} (see Section~\ref{sec:properties_and_flavours:def-use})
relationships and \emph{live ranges} (see Section~\ref{sec:properties_and_flavours:domprop}),
...
Chapters~\ref{chapter:classical_construction_algorithm} and~\ref{chapter:alternative_ssa_destruction_algorithm} present efficients and effective SSA destruction algorithms. \\ \hline
\end{tabular}
\end{center}
