...
\ForEach{$d \in v$.defs}{
    $b \gets d$.block\;
    insert $d$ in $b$.defs according to schedule\tcc*{latest def. is first in list}
  }

  \ForEach{use $(u, \mbox{index})$ of $v$}{
  $d \gets \mbox{None}$\;
    $b \gets u.\mbox{block}$\;
    \If{u is a \phifun}{
      $b \gets u.\mbox{block}.\mbox{pred}(\textit{index})$\;
      $d \gets \mbox{FindDefFromEnd}(b)$\;
    }
    \Else{
      \ForEach{$l \in b.\mbox{defs}$ in order}{
      \If{$l.\mbox{order} < u.\mbox{order}$}{\label{l:before-use}
          $d \gets l$\;
          break\;
        }
        ...
        \If{not $d$}{
          $d \gets \mbox{FindDefFromBegin}(b)$\;
        }
      }
    }
...
\begin{procedure}
  ...
  \Else{
    \Return{\mbox{FindDefFromBegin}(b)}
  }
\end{procedure}
...
\begin{procedure}
  ...
  \KwIn{$b$, a basic block}
  \If{$b \in \textnormal{IDF}$}{
    $d \gets \mbox{new\_phi}(b)$\;
    append $d$ to $b.\mbox{defs}$\;
%    $i \gets 0$\;
    \ForEach{$p \in b.\mbox{preds}$}{ 
      $o \gets \mbox{FindDefFromEnd}(p)$\;
%      set $i$-th operand of $d$ to $o$\;
      set corresponding operand of $d$ to $o$\;
%      $i \gets i + 1$\;
    }
  }
  \Else{
  $d \gets \mbox{FindDefFromEnd}(b.\mbox{idom})$\tcc*{search in immediate dominator}
  }
  \Return{d}
\end{procedure}
...
\If{$b.\textnormal{beg} \ne \mbox{None}$}{
  ...
  \ForEach{$p \in b.\textrm{preds}$}{
    reaching\_defs $\gets$ reaching\_defs $\cup$ FindDefFromEnd$(p)$
  }
...
\tcc{Checks if the parameter list in reaching\_defs makes
...
\KwOut{$v_\phi$ if \pendphi is necessary, the variable to use instead otherwise}
  ...
  \ForEach{$d \in \textrm{reaching\_defs}$}{
    \lIf{$d = v_\phi$}{
      continue\;
    }
    \If{other = None}{
