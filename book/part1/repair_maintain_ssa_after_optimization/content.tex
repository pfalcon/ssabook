...
\index{insertion of \phifun, pessimistic}
...
The first algorithm (Choi et al.~\cite{Choi:1996ji}) is based on the iterated dominance frontiers like the classical SSA construction algorithm by Cytron et al.~\cite{cytron:1991:ssa}.
...
The second algorithm is based on the algorithm by Braun et al.~\cite{DBLP:conf/cc/BraunBHLMZ13} which is an extension of the construction algorithm that Click describes in his thesis~\cite{click:thesis} to construct SSA from an abstract syntax tree.
...
\index{reducible CFG}\index{minimal, making SSA}
Both approaches construct \emph{pruned}\index{pruned SSA form} SSA, i.e., they do not add dead \phifuns.
...
Such a non-local check also eliminates unnecessary \phifuns in the irreducible case~\cite{DBLP:conf/cc/BraunBHLMZ13}.
