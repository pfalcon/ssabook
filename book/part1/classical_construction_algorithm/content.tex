...
the dominance property, i.e.\ where each 
...
\begin{figure}
  \subfloat[CFG]{
    \includegraphics{iDF-CFG.pdf}
  }
  \qquad
  \subfloat[Graphs]{
    \includegraphics{iDF-Graphs.pdf}
  }
...
\ForEach{ B : basic Block in depth-first search preorder traversal of the dominance tree}{
  ...
  \ForEach{f: \phifun\ in a successor of B}{
      \ForEach{ v : variable used as a source operand on right-hand
        side of f}{
...
\ref{sec:properties_and_flavours:domprop}.
...
Since freshly constructed, untransformed SSA code is conventional, its destruction is straightforward.
...
\phiweb is interference free. In particular if each \phiweb's constituent
...
The simplest (although not the most efficient) way to destroy non-conventional SSA form is to split all \textit{critical edges}, and then replace \phifuns\ by parallel copies at the end of predecessor basic blocks.
...
As already mentioned, SSA destruction of such form is straightforward. However Algorithm \ref{alg:ssadestruction:splitting} can be slightly modified to directly destruct SSA by: removing line~\ref{line:api}; replacing $a'_i$ by $a_0$ in the following lines; adding ``remove the \phifun'' at line~\ref{line:removephi}.
 ...
 \ForEach{$B$: basic block of the CFG}{
   ...
   \ForEach{\phifun\ at the entry of $B$ of the form $a_0=\phi(B_1:a_1,\ldots,B_n:a_n)$}{
     \ForEach{$a_i$ (argument of the \phifun\ corresponding to $B_i$)}{
       \nl\label{line:api} \Let{$a'_i$ be a freshly created variable}\; 
       add copy $a'_i \leftarrow a_i$ to $\mathrm{PC}_i$\;
       replace $a_i$ by $a'_i$ in the \phifun\;
     }
     \nl\label{line:removephi} \BlankLine
  }
}
}
...
\ForEach{I : instruction of the CFG in dominance order}{
  \If{I is \phifun\ defining variable $a$}{
    mark $a$ as useless\;
  }
  \Else{ \tcc{I is not \phifun}
    \ForEach{x : source operand of I}{
      \If{x is defined by \phifun}{
        mark $x$ as useful\;
        $\textit{stack}.\mathrm{push}(x)$\;
      }
    }
  }
}
...
\While{$\textit{stack}$ not empty}{
  ...
  \ForEach{$x$ : source operand of I}{
    \If{$x$ is marked as useless}{
      \tcc{$x$ marked as useless implies $x$ defined by \phifun}
      mark $x$ as useful\;
      $\textit{stack}.\mathrm{push}(x)$\;
    }
  }
...
\begin{figure}
  \includegraphics{t1_t2_transforms.pdf}
