\chapter{Standard Construction and Destruction Algorithms \Author{J.~Singer \andAuthor F.~Rastello}}
\label{chapter:classical_construction_algorithm}
\inputprogress

\graphicspath{{Figures/}{classical_construction_algorithm/Figures/}{part1/classical_construction_algorithm/Figures/}{tikz/}{classical_construction_algorithm/tikz/}{part1/classical_construction_algorithm/tikz/}}
\inputpath{{tikz/}{classical_construction_algorithm/tikz/}{part1/classical_construction_algorithm/tikz/}}

{ 
\index{construction, of SSA}
\index{destruction, of SSA}
\index{out-of-SSA}
...
The set of variables is $\{ x, y, \textit{tmp} \}$.
...
\begin{figure}
  \includegraphics[scale=0.9]{initial1.pdf}
...
More formally,
\begin{itemize}
\item node $a$ \textit{strictly dominates}\index{dominates, strictly} node $b$ if $a$ dominates
  $b$ and $a \neq b$
...
The \textit{iterated dominance frontier} $\iDF(S)$,\index{iterated dominance frontier, \iDF}
...
the dominance property\index{dominance property}, i.e.\ where each 
renamed variable's definition dominates its entire live-range.
...
frontier of each CFG node is pre-computed and that the iterated dominance frontier is computed just-in-time, as the algorithm proceeds.
...
Effectively, this is just-in-time calculation of
the iterated dominance frontier.
%
The flags array $F$ is used to avoid repeated insertion of \phifuns\
...
\Begin{
 ...
 $F \leftarrow \{ \}$ : set of basic blocks\;
...
\caption{\label{alg:classical_construction:phi_insertion}Standard algorithm for inserting $\phi$-functions for  a variable $v$}
\end{algorithm}
...
\begin{figure}
  \includegraphics[scale=0.9]{initial2.pdf}
...
of the dominance frontier, then we can define the \iDF-graph as the transitive closure of the \DF-graph. In our example, as $\{(C,E),\ (E,G)\}$ are \DF-edges, $(C,G)$ is a \iDF-edge. Hence, a definition of $x$ in $C$ will lead to inserting \phifuns in $E$ and $G$.
...
\begin{figure}
  \subfloat[CFG]{
    \includegraphics[scale=0.65]{iDF-CFG.pdf}
  }
  \subfloat[DJ-Graph]{
    \includegraphics[scale=0.7]{iDF-J.pdf}
  }
  \subfloat[\DF-Graph]{
    \includegraphics[scale=0.7]{iDF-DF.pdf}
  }
  \subfloat[\iDF-Graph]{
    \includegraphics[scale=0.7]{iDF-iDF.pdf}
  }
\caption{\label{fig:classical_construction_algorithm:iDF}An example CFG and it corresponding DJ-graph ($D$-edges are highlighted), \DF-graph and \iDF-graph. $\iDF(C)=\{E,G\}$}
\end{figure}

\begin{algorithm}
\Begin{
 \For{$(a,b) \in \mathrm{cfgEdges}$}{
  \While{a does not strictly dominate b}{
...
It is conventional to treat each variable use in a \phifun\
...
% basic blocks,
then def-use chains\index{def-use chain} are aligned with the CFG dominator tree.
In other words,
...
called a \emph{version}\index{version, variable's}\index{name, variable's}.
...
\ForEach{ \textit{BB} : basic Block in depth-first search preorder traversal of the dominance tree}{
  \ForEach{ i : instruction in linear code sequence of \textit{BB}}{
      \ForEach{v : variable used as a source operand on right-hand
        side of  non-\phifun\ i}{
        $\mathrm{updateReachingDef}(v,i)$\;
        replace this use of $v$ by $\reachingDef{v}$ in $i$\;
      }
      \ForEach{ v : variable defined by i, which may be a \phifun}{
        $\mathrm{updateReachingDef}(v,i)$\;
  ...
  \ForEach{f: \phifun\ in a successor of \textit{BB}}{
      \ForEach{ v : variable used as a source operand on right-hand
        side of f}{
...
\begin{figure}
\subfloat[CFG]{
  \includegraphics[scale=0.9]{initial3.pdf}
\label{fig:classical_construction_algorithm:renaming}
}\hfill
\subfloat[Walktrough of renaming for variable $x$ in example CFG]
{\begin{tabular}[b]{|c|c|l|}\hline
\textit{BB} & $x$ mention &  $\reachingDef{x}$ \\ \hline \hline
$r$ & use at $l_1$ & $\bot$\\
...
$E$ & use at $l_8$ & $x_6$\\ \hline
\end{tabular}\label{table:classical_construction_algorithm:renaming}
}
\caption{SSA form of the example of Figure~\ref{fig:classical_construction_algorithm:examplecfg}}
\end{figure}
...
\caption{updateReachingDef(v,i) Utility
  function for SSA renaming\label{alg:classical:updateRD}}\index{reaching definition}
\end{procedure}
...
may be dead(e.g. $y_5$ in Figure~\ref{fig:classical_construction_algorithm:renaming}), i.e.\ there is no explicit use of the variable subsequent
to the \phifun.
...
the $\undef$ value used to initialize each reachingDef slot in Algorithm\index{undefined variable}
...
Since freshly constructed, untransformed SSA code is conventional, its destruction is straightforward.\index{conventional SSA form}
...
\phiweb is interference free. In particular if each \phiweb's constituent\index{conventional SSA}
...
As explained further, the replacement of parallel copies by sequences of simple copies is handled later on.
...
As already mentioned, SSA destruction of such form is straightforward. However Algorithm~\ref{alg:ssadestruction:splitting} can be slightly modified to directly destruct SSA by: removing line~15; replacing $a'_i$ by $a_0$ in the following lines; adding ``remove the \phifun'' at line~19.
...
Alternatively, 
pruning SSA form is equivalent to a dead-code elimination pass\index{dead \phifuns}\index{dead code elimination}\index{def-use chain}
...
many variables are \emph{local}\index{local variable}, i.e. have a single live-range that is
...
can removes many of these redundant \phifuns. 
...
\begin{figure}
  \includegraphics[scale=0.9]{t1_t2_transforms.pdf}
