...
\inputprogress
\numberofpages{10}
\graphicspath{{img/}{semantics/}{part1/semantics/}}
...
\item to make syntactic conditions and semantic invariants that are
  ...
  way, functional representations of SSA explicitly enforce invariants
  such as ``all $\phi$-functions in a block must be of the same
  arity'', ``the variables assigned to by the $\phi$-functions in a
  block must be distinct'', or ``$\phi$-functions are only allowed to
  occur at the beginning of a basic block''. Reasoning using
...
\subsubsection{Low-level functional program representations}
...
A consequence of referential transparency is \emph{compositional
equational reasoning}, i.e.~the property that replacing a phrase by
some semantically equivalent phrase leaves the meaning of the
surrounding code unchanged. Indeed, the compatibility with equational
reasoning is a central property of functional languages. Of course,
semantic preservation is a core requirement on transformations
implemented in a compiler. Under this persepctive, the suitability of
SSA for formulating and implementing program transformations can be
explained by the proximity of SSA to functional languages.
...
\subsubsection{Functional construction of SSA}
