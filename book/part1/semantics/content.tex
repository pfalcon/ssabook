...
\item\label{ParameterDroppingConditionTwo} The tightest scope for $x$
  ...
  of $f$) is the one associated with the formal parameter $x$ in the declaration of $f$.
\end{enumerate}
...
\end{figure}
The reason that this $\phi$-function can't be eliminated is precisely the 
reason why $y$ survives parameter-dropping (i.e., the fact that $y$ is 
redefined in the loop).
...
let-bound in the host body of the function.
...
SSA. For example, in loop-closed\index{loop-closed} SSA form~(see
...
is declared as $\mathtt{function}\ f(x,y,z) = e$ may be converted to\par
\vskip-3pt
...
edge from one of dominance successors of $b_i$ (i.e., the descendants of
...
notions of \emph{loop nesting forests}. Of the various loop nesting 
forest
...
$G_0$. Removing the back-edges of $L_0$ (i.e., edges from $B_0$ to $H_0$)
from $G_0$ yields $G_1$, whose (only) SCC determines a further inner
loop $L_1 = (\{w,x\},
\{w,x\})$. Removing the back-edges $L_1$ from $G_1$ results in the
...
Figure~\ref{sub:ramal-steens-overlayLNF}. Loops
                       ...
                       (%(*function\ u}\
                        ...
                        \ , \lambda p_v.\ \funbody{\mathit{body\ of}\ v,\ 
                                            \mathit{with\ call\ to}\ 
                                               x}\ )
                      \end{array}\\
                      \mathtt{end}\ \funcomment[34mm]{\mathit{end\ of\ outer\ loop}}
                    \end{array}\\
...
style articles by Reynolds and Plotkin \cite{Reynolds:1972,Reynolds1974,Plotkin75}.  Two
