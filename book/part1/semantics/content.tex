...
implicit syntactic conditions and semantic invariants that motivate
the definition of SSA and guide its use. Indeed, the introduction of
...
of the present chapter is to highlight aspects of the basic
correspondence and then point the reader to some more advanced
material.

\paragraph{Chapter outline}
We first highlight the main ideas of two low-level functional language
formats that may be used to represent SSA, namely
\emph{continuation-passing} and \emph{direct} style. With
this background in place, we then discuss the relationship between SSA
and functional forms in more detail by describing how various steps
that occur during the construction and destruction of SSA mirror
...
work, before concluding with some brief pointers to alternative
...
\subsection{Variable assignment versus name binding}
...
\footnote{O'Donnell~\cite{ODonnellPhD} and Kelsey~\cite{Kelsey95} observed that
t the correspondence bwtween let-binding and points of varibale declaration extends to other aspects of program structure.

CPS compilers: \cite{DBLP:journals/lisp/SussmanS98a,Appel:CWC}.

Appel~\cite{Appel98:SSA} popularized the correspondence to SSA to
direct style, which itself was introduced
Reynolds~\cite{Reynolds1974}.  } The correspondence between SSA and
functional languages was observed shortly after the introduction of
SSA, and pioneered by O'Donnell, Kelsey, and
Appel~\cite{ODonnellPhD,Kelsey95,Appel98:SSA}.
...
Lambda dropping~\cite{DBLP:journals/tcs/DanvyS00}, lambda-lifting~\cite{DBLP:conf/fpca/Johnsson85}
