...
\subsubsection{Loop nesting forests}
\label{section:semantics:loopnestingforests}
As discussed in
Chapter~\ref{chap:alternative_ssa_construction_algorithms}, loop
...
Code~(\ref{FuntionalLoopNest1}) may be improved in two respects. The
...
application of block sinking:
\begin{equation}
\label{FuntionalLoopNest2}
\begin{array}{l}
\mathtt{function}\ f_1(\param 1)\ = \\
  \quad
  \begin{array}{l}
...
code~(\ref{FuntionalLoopNest2})) is the position of node $12$. In
...
$\loopfun_2$ and $\loopfun_9$ as in code~(\ref{FuntionalLoopNest1}),
we insert calls to $f_2$ and $f_9$, as highlighted in
code~(\ref{FuntionalLoopNest3}):
\begin{equation}
\label{FuntionalLoopNest3}
\begin{array}{l}
\mathtt{function}\ f_1(\param 1)\ = \\
  \quad
  \begin{array}{l}
...
(\ref{FuntionalLoopNest3}) into the CFG from Figure~\ref{fig:lnf}(a)
...
sinking to code~(\ref{FuntionalLoopNest3}) yields code with the
nesting structure of~(\ref{FuntionalLoopNest2}) but the call structure
of~(\ref{FuntionalLoopNest3}). Alternatively, we may obtain this code
by altering the loopback calls in the bodies of $f_{12}$ and $f_{11}$
of~(\ref{FuntionalLoopNest2}) to target directly the functions $f_2$
...
of loopback edges in the style of~(\ref{FuntionalLoopNest3}) it is in
...
to~(\ref{FuntionalLoopNest1}) and ~(\ref{FuntionalLoopNest2}), a
...
code~(\ref{FuntionalLoopNest2}) by appropriately indicating the three
definitions and the single use of $v$ yields
code~(\ref{FuntionalLoopNest2v}), under the simplifying assumption
that no other variables are present.
\begin{equation}
\label{FuntionalLoopNest2v}
\begin{array}{l}
\mathtt{function}\ f_1()\ = \\
  \quad
  \begin{array}{l}
