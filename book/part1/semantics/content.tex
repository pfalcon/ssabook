...
\inputpath{part1}{semantics}
\inputprogress

{

\let\oldmathtt=\mathtt
%\def\mathtt#1{{\bf \oldmathtt{#1}}}
\let\mathtt=\mathbf

\newcommand{\pdec}[3]{\ensuremath{\mathtt{proc}\ } {#1}\ \mathtt{(}{#2}\mathtt{)\ =} \ {#3}}
\newcommand{\mathlet}{\ensuremath{\mathtt{let}}}
...
\newcommand{\funbody}[1]{\ldots<~\mathit{#1}~>\ldots}
\newcommand{\funcomment}[2][0pt]{%
  \kern#1{\color{commentcolor}\rhd~#2}%
  %\kern#1{\color{commentcolor}(*~#2~*)\xspace}%
}
...
robustness, maintainability, and code readability of the compiler;
...
%constant propagation algorithm~\cite{WZ91}. In the same
...
parameters\index{formal parameters} $x_i$ and the function name $f$ within 
$e$.
...
\emph{let-binding}\index{let-binding}: {\small $$\letin x {e_1} {e_2}.$$} The 
...
by this binding, $e_2$, is called the \emph{static scope}\index{scope!static} 
...
\quad 
  \fbox{$
   \begin{array}[t]{l} 
    \mathtt{let}\ y = (\mathtt{let}\ v=2\times v \ \mathtt{in}\ \fbox{$4\times v$}_v\ \mathtt{end})\\
    \mathtt{in}\ \fbox{$y\times v$}_{v,y}\ \mathtt{end}\\
\end{array}$}_v\\
\mathtt{end}
\end{array}
...
$2\times 3=6$ shadows the outer binding of $v$ to value $3$ precisely
for the duration of the evaluation of the expression $4\times v$. Once
this evaluation has terminated (resulting in the binding of $y$ to
$24$), the binding of $v$ to $3$ becomes visible again, yielding the
...
called \emph{free}\index{variable!free}\index{free|see{variable}}.  A
well-formed procedure declaration contains all free variables of its
body amongst its formal parameters.  Thus, the notion of scope makes
explicit the invariant that each use of a variable should be
...
transparency}\index{referential transparency}:
...
\quad 
  \fbox{$
   \begin{array}[t]{l} 
    \mathtt{let}\ y = (\mathtt{let}\ z=2\times v \ \mathtt{in}\ \fbox{$4\times z$}_{v,z}\ \mathtt{end})\\
    \mathtt{in}\ \fbox{$y\times v$}_{v,y}\ \mathtt{end}\\
\end{array}$}_v\\
\mathtt{end}
\end{array}
...
variables is avoided. Formally, this means that a renaming
{\small $$
  \letin x
...
%  \fbox{$
%   \begin{array}[t]{l} 
%    \mathtt{let}\ y = (\mathtt{let}\ y=2\times v \ \mathtt{in}\ \fbox{$4\times y$}_{v,y}\ \mathtt{end})\\
%    \mathtt{in}\ \fbox{$y\times v$}_{v,y}\ \mathtt{end}\\
%\end{array}$}_v\\
%\mathtt{end}
%\end{array}
...
%$y$, namely the expression $y\times v$, contains the variable $v$ free, we
...
called $\alpha$-renaming\index{alpha-renaming@$\alpha$-renaming}. Typically, 
...
equational reasoning}\index{compositional equational reasoning}:
the meaning of a piece of code $e$ is only
dependent of its free variables, and can be calculated from the meaning
of its subexpressions. For example, the meaning of a phrase {\small $\letin x
...
%precise way to
\emph{continuation-passing-style}\index{continuation passing style}\index{CPS|see{continuation}}
(CPS), a program representation routinely used in compilers for
functional languages~\cite{Appel:CWC}.
...
\begin{array}{l}
  \mathtt{let}\ v = 3\ \mathtt{in}\\
  \quad \begin{array}{l} 
    \mathtt{let}\ y = (\mathtt{let}\ v=2\times v \ \mathtt{in}\ 4\times v\ \mathtt{end})\\
    \mathtt{in}\ k (y\times v)\ \mathtt{end}
  \end{array}\\
  \mathtt{end}
  \end{array}
\end{functional}%
...
$\lambda$-notation\index{lambda-notation@$\lambda$-notation}, i.e., in the form 
...
\begin{functional}
\begin{array}{l}
\mathtt{let}\ k = \lambda\, x .\, 2\times  x\\ 
\mathtt{in}\ 
  \begin{array}[t]{l}
    \mathtt{let}\ v = 3\ \mathtt{in}\\
    \quad \begin{array}{l} 
      \mathtt{let}\ y = (\mathtt{let}\ z=2\times v \ \mathtt{in}\ 4\times z\ \mathtt{end})\\
      \mathtt{in}\ k (y\times v)\ \mathtt{end}
    \end{array}\\
    \mathtt{end}
  \end{array}\\ 
...
When the continuation $k$ is applied to the argument $y\times v$, the
...
\mathtt{in}\
  \begin{array}{l}
  \mathtt{let}\ k = \lambda\, x .\, 2\times  x\ \mathtt{in}\ f(k)\ \mathtt{end}
  \end{array}\\
\mathtt{end}.
\end{array}
\end{functional}%
...
%\end{functional}%
%where 
This makes CPS form a discipline of programming using higher-order
functions, as continuations are constructed ``on-the-fly'' and
communicated as arguments of other function calls.  Typically, the caller
of $f$ is itself parametric in \emph{its} continuation, as in
\begin{functional}
...
\quad
\begin{array}{l}
\mathtt{let}\ k' = \lambda\, x .\, k (x + 7)\ \mathtt{in}\ f(k')\ \mathtt{end}.
\end{array}
\end{array}
\end{functional}%
...
\quad
  \begin{array}{l}
    ...
    \quad \begin{array}{l}
            \mathtt{let}\ k' = \lambda\, z .\, k (z\times x)\\
            \mathtt{in}\
               \begin{array}[t]{l}
                 \mathtt{if}\ y>0\\
                 \mathtt{then\ let}\ z = y\times 2\ \mathtt{in}\ k'(z)\ \mathtt{end}\\
...
\begin{center}
  ...
  \subfloat[]{
    \tikzsubfigure[1]{FunctionalCodeCFG}
    \label{sub:funcCFG}
  }
  \qquad
  \subfloat[]{
    \tikzsubfigure[2]{FunctionalCodeCFG}
    \label{sub:funcSSACFG}
  }
\end{center}
\caption{Control-flow graph for code~(\ref{ContinuationCode4}) 
\protect\subref{sub:funcCFG}, and SSA representation \protect\subref{sub:funcSSACFG}.}
  ...
  \begin{array}{l}
    ...
    \quad \begin{array}{l}
            ...
            \mathtt{in}
               \begin{array}[t]{l}
                 \mathtt{if}\ y>0\\
                 \mathtt{then\ let}\ z_1 = y\times 2\ \mathtt{in}\ k'(z_1)\ \mathtt{end}\\
...
additional function arguments is the \emph{direct style}\index{direct style}, 
in which we represent code as a set of locally
named tail-recursive\index{tail-recursive} functions, for which the last 
...
\quad
  \fbox{
    $\begin{array}{l}
       ...
       \quad \fbox{
               $\begin{array}{l}
                  \mathtt{function}\ h' (z) = \fbox{$z\times x$}_{x,y,z,h,h'} \\
                  \mathtt{in}\
                    \fbox{
                     $\begin{array}[t]{l}
                        \mathtt{if}\ y>0\\
                        \mathtt{then\ let}\ z = y\times 2\ \mathtt{in}\ 
                  ...
                  \mathtt{then}\ 
                     \begin{array}[t]{l}
                        \mathtt{function}\ h_1 () = \mathtt{let}\
                              z = y\times 2\ \mathtt{in}\ h'(z)\ \mathtt{end}\\
            ...
            \mathtt{in}\
                \begin{array}[t]{l}
                  ...
                  \mathtt{then}\ 
                     \begin{array}[t]{l}
                        \mathtt{function}\ h_1 () = \mathtt{let}\
                              z_1 = y\times 2\ \mathtt{in}\ h'(z_1)\ \mathtt{end}\\
                        \mathtt{in}\ h_1()\ \mathtt{end}
                     \end{array}\\
                  \mathtt{else}\ 
                     \begin{array}[t]{l}
...
pronounced for code in \emph{let-normal form}\index{let-normal form}:
each intermediate
...
subject to the side condition that $y$ is not free in $e''$.
...
\quad 
  {\fbox{
   $\begin{array}[t]{l} 
     \mathtt{let}\ z=2\times v \ \mathtt{in}\\
     \quad {\fbox{$\mathtt{let}\ y = 4\times z\ \mathtt{in}\ 
                 \fbox{$y\times v$}_{v,y,z}\ \mathtt{end}$}_{v,z}}\\
     \mathtt{end}
   \end{array}$}_{v}}\\
\mathtt{end}
\end{array}
...
%& $\mathtt{z:=8}$ & & & $\mathtt{y=y\times z}$ & & & $\mathtt{return\ w}$\\
...
%\includegraphics[scale=0.5]{SSAConstructionExample1}
  \tikzsubfigure[1]{SSAConstructionExample}
\end{center}
...
\begin{array}{l}
  ...
  \begin{array}{ll}
    \mathtt{and}\ f_2(v,z,y) =
     & \mathtt{let}\ x = {5+y},\ y = {x\times z},\ x = {x-1}\\
...
\item
  Each subterm $e_2$ of a let-binding {\small $\letin x {e_1} {e_2}$}
...
%\includegraphics[scale=0.4]{SSAConstructionExample2}
  \subfloat[CFG]{
    \tikzsubfigure[2]{SSAConstructionExample}
  }
  %\quad
  \subfloat[Dominance tree]{
    \qquad
    \qquad
    \tikzsubfigure[1]{SSAConstructionExample-domTree}
    \qquad
  }
\end{center}
\caption{Pruned (non-minimal) SSA form.}
\label{fig:FunctionalCorrespondenceRunningExampleInitialSSA}
  \evenoddcheck{semantics:pruned-ssa}
\end{figure}
...
$\lambda$-dropping\index{lambda-dropping@$\lambda$-dropping}\index{dropping!$\lambda$} 
...
Block sinking\index{block!sinking} analyzes the static call structure to 
identify which
...
\item\label{ParameterDroppingConditionTwo} The tightest scope for $x$
  ...
  of $f$) is the one associated with the formal parameter $x$ in the
  declaration of $f$\!.
\end{enumerate}
The rationale for these clauses is that removing $x$ from $f$'s
parameter list means that any free occurrence of $x$ in $f$'s body is
now bound outside of $f$'s declaration. Therefore, for each call to
$f$ to be correct, one needs to ensure that this outside binding
coincides with the one containing the call. 
...
In code (\ref{FunctionalConstructionProgram4}), these conditions sanction
...
recursive call is in the scope of the let-binding for $y$ in the body of $f_2$,
preventing us from removing $y$.  In contrast, neither $v$ nor
...
desired code with a single $\phi$-function, for variable $y$ at the
beginning of block $b_2$, see
Figure~\ref{fig:FunctionalCorrespondenceSSAofLambdaDroppedCode}.
\begin{figure}
\begin{center}
%  \includegraphics[scale=0.4]{SSAConstructionExample3}
  \subfloat[CFG]{
    \tikzsubfigure[3]{SSAConstructionExample}
  }
%  \quad
  \subfloat[Dominance tree]{
    \qquad
...
\end{center}
\caption{SSA code after $\lambda$-dropping.}
\label{fig:FunctionalCorrespondenceSSAofLambdaDroppedCode}
\end{figure}
The reason that this $\phi$-function can't be eliminated (the redefinition of 
...
let-bound in the body of the host function.
...
if all non-recursive calls to $f_i$ come from within $f_j$, i.e., exactly if
all paths from the initial program point to block $b_i$ traverse $b_j$, i.e.,
...
SSA. For example, in loop-closed\index{loop-closed|exampleidx} SSA form~(see
...
obtained in the final iteration of the loop.  In our example, the only
...
\begin{center}
  ...
  \subfloat[Loop-closed]{
    \tikzsubfigure[4]{SSAConstructionExample}
    \qquad
    \tikzsubfigure[1]{SSAConstructionExample-domTree}
    \label{sub:loop-closed}
  }

  \subfloat[Loop-unrolled]{
  %% use of boxes to center vertically
    \newsavebox\tempbox
    \sbox{\tempbox}{\tikzsubfigure[5]{SSAConstructionExample}}
    \usebox{\tempbox}
    \qquad
    \vbox to \ht\tempbox{%
      \vfil
      {\hbox{\tikzsubfigure[2]{SSAConstructionExample-domTree}}
      }
      \vfil
    }
    \label{sub:loop-unrolled}
  }
\end{center}
\caption{Loop-closed 
\protect\subref{sub:loop-closed}
and loop-unrolled 
\protect\subref{sub:loop-unrolled}
forms of running example program,
...
\label{fig:FunctionalCorrespondenceSSAofLoopClosedProgramNEW} 
\end{figure}
...
in Figure~\ref{sub:loop-closed}. The nesting structure of
...
We unroll\index{unrolling|exampleidx} the loop by duplicating the body of 
$f_2$, \emph{without
...
this code---shown in
Figure~\ref{sub:loop-unrolled}---the first instruction in $b_3$ has turned into a
non-trivial $\phi$-node. As expected, the parameters of this
...
imperative non-SSA form. Thus, the task of SSA destruction\index{destruction!SSA}\index{SSA!destruction} amounts to
amounts to
...
of the form {\small $\letin x y e$}. For example, a call $f(v,z,y)$ where $f$
is declared as {\small $\mathtt{function}\ f(x,y,z) = e$} may be converted to\par
\vskip -6pt
\begin{functional*}
\mathtt{let}\ x = v,\ a = z,\ z = y,\ y = a\ \mathtt{in}\ {f(x,y,z)}
\end{functional*}%
in correspondence to the move instructions introduced in imperative
% $$\small \mathtt{let}\ x = v,\ a = z,\ z = y,\ y = a\ \mathtt{in}\ 
% {f(x,y,z)}$$
in correspondence to the move instructions introduced in imperative 
...
$\alpha$-renaming and thus only syntactically a functional language. For 
example, we can give a local algorithm that considers each call site 
individually and avoids the ``lost-copy'' and ``swap'' problems (cf.~Chapter~\ref{chapter:alternative_ssa_destruction_algorithm})\index{lost-copy problem}\index{swap problem}:
Instead of introducing let-bindings for all parameter positions of a call, the 
algorithm scales with the number and size of cycles that span identically 
named arguments and parameters (like the cycle between $y$ and $z$ above), and 
employs a single additional variable (called $a$ in the above code) to break 
all these cycles one by one.


%%%%%%%%%%%%%%%%%%%%%inclusion of {ShortLoopanalysis}
...
\begin{array}{l}
  ...
  \mathtt{let} \funbody{body~of~b} \mathtt{in}\\
  \quad \begin{array}{l}
          \begin{array}{rcl}
            \mathtt{function}\ f_1(\ldots) & = & e_1\
            \funcomment[2em]{\mathit{body\ of}\ b_1, \mathit{with\ calls\ to}\ b, b_i}\\[-1ex]
             \vdots \\
            \mathtt{and}\ f_n(\ldots) & = & e_n
                  \funcomment[2em]{\mathit{body\ of}\ b_n, \mathit{with\ calls\ to}\ 
                      b, b_i}
          \end{array}\\
          \mathtt{in} \funbody{\mathit{calls\ to}\ b, b_i~from~b}
            \mathtt{end}
        \end{array}
  \end{array}
\end{functional}%
By exploiting additional control-flow structure between the $b_i$, it
is possible to obtain refined placements, namely placements
...
adding arrows $b_i \to b_j$ whenever the CFG contains a directed edge 
from one of the dominance successors of $b_i$ (i.e., the 
descendants of $b_i$ in the dominance tree) to $b_j$.

In the case of a \emph{reducible}\index{reducible CFG} CFG, the resulting graph contains
...
\begin{center}
  ...
  \hfill
  \subfloat[CFG]{
    \tikzsubfigure[1]{shortLoopAnalysis}
    \label{sub:shortloop-cfg}
  }
  \hfill
  \subfloat[Dominance tree]{
    \tikzsubfigure[2]{shortLoopAnalysis}
    \label{sub:shortloop-domtree}
  }
  \hfill\null
\end{center}
\caption{Example placement for reducible flow graphs.
  \protect\subref{sub:shortloop-cfg} Control-flow graph;
  \protect\subref{sub:shortloop-domtree} Overlay of CFG
...
Figure~\ref{sub:shortloop-cfg} and its enriched
dominance tree shown in
Figure~\ref{sub:shortloop-domtree}. A possible (but
not unique) ordering of the children of $b$ is $[b_5,b_1,b_3,b_2,b_4]$,
resulting in the nesting shown in code~(\ref{FunctionalCascadeFun}).
\begin{functional}
\label{FunctionalCascadeFun}
\begin{array}{l}
  \mathtt{function}\ f (\ldots)= 
  \mathtt{let} \funbody{\mathit{body\ of}\ f} \mathtt{in}\\
  \quad \begin{array}{l}
          \mathtt{function}\ f_5(\ldots) = e_5\\ \mathtt{in}\  
          \begin{array}[t]{l}
            \mathtt{function}\ f_1(\ldots) = e_1\ 
                 \funcomment[15mm]{\mathit{contains\ calls\ to}\ f\ \mathit{and}\ f_5} \\
            \mathtt{in}\ 
            \begin{array}[t]{l}
              ...
              \begin{array}[t]{l}
                ...
                \qquad
                  \begin{array}[t]{l}
                    \mathtt{let} \funbody{\mathit{body\ of}\ f_2}\\
                    \mathtt{in}\
                      \begin{array}[t]{l}
                        \mathtt{function}\ g(\ldots) = e_g
                           \funcomment[7mm]{\mathit{contains\ call\ to}\ f_3}\\
                        \mathtt{in}\ \funbody{\mathit{calls\ to\ f_5}\ \mathit{and}\ g}
                         \mathtt{end}
                     \end{array}\\
                   \end{array}\\ 
                \mathtt{in}\
                \begin{array}[t]{l}
                  \mathtt{function}\ f_4(\ldots) = e_4
                    \funcomment[3.2mm]{\mathit{contains\ calls\ to}\ f_3\ 
                        \mathit{and}\ f_4} \\
                   \mathtt{in}
                      \funbody{\mathit{calls\ to}\ f_1, f_2, f_4, f_5}
                   \mathtt{end}
                \end{array}
              \end{array}
            \end{array}
...
Declaring functions using $\lambda$-abstraction brings further 
improvements.
This enables us not only to syntactically
distinguish between loops and non-recursive control-flow structures
using the distinction between $\mathtt{let}$ and 
$\mathtt{letrec}$\index{letrec}
...
$\mathtt{let}$/$\mathtt{letrec}$ bindings with pattern matching, if we
...
\begin{array}{l}
  ...
  \quad \begin{array}{l}
          ...
          \begin{array}[t]{l}
            ...
            \funcomment[18mm]{\mathit{contains\ calls\ to}\ f\ \mathit{and}\ f_5} \\
            \mathtt{in}\ 
            \begin{array}[t]{l}
               ...
               \quad 
                 \begin{array}{l}
                   ...
                   (\lambda\; p_2.\; 
                       \begin{array}[t]{l}
                         \mathtt{let}
                            \funbody{\mathit{body\ of}\ f_2}
                         \mathtt{in}\\
                         \quad \begin{array}{l}
                           \mathtt{let}\ g = \lambda\; p_g.\; e_g\
                              \funcomment[5.4mm]{\mathit{contains\ call\ to}\ f_3}\\ 
                           \mathtt{in}
                                 \funbody{\mathit{calls\ to}\ f_5\ 
                                     \mathit{and}\ g}
                           \mathtt{end}
                         \end{array}
                       \end{array}\\
                    \;, \lambda\; p_4.\; e_4)
                       \funcomment[14.4mm]{\mathit{contains\ calls\ to}\ f_3\ \mathit{and}\ f_4}
                  \end{array}\\
               \mathtt{end}\ \funcomment[10mm]{\mathit{declaration\ of\ tuple}\ (f_2,f_4)\
                                 \mathit{ends\ here}} 
            \end{array}\\
            \mathtt{in}
               \funbody{\mathit{calls\ to}\ f_1, f_2, f_4, f_5}
...
In the case of \emph{irreducible}\index{irreducible CFG} CFGs, the
enriched dominance tree is no longer acyclic (even when ignoring
self-loops). In this case, the functional representation not only
depends on the chosen DFS order but additionally on the partitioning
of the enriched graph into loops.  As each loop forms a \emph{strongly
  connected component} (SCC), different partitioning are possible,
corresponding to different notions of \emph{loop nesting
  forests}\index{loop nesting forest}. Of the various loop nesting
forest strategies proposed in the
SSA-literature~\cite{Ramalingam:2002:TOPLAS}, the scheme introduced by
Steensgaard~\cite{SteensgardMSR-TR-93-14} is particularly appealing
from a functional perspective.

%\begin{description}
%\item[Steensgaard's] 
In Steensgaard's notion of loops, the headers $H$ of a loop $L=(B,H)$
are precisely the entry nodes of its body $B$, i.e., those nodes in $B$
that have a predecessor outside of $B$.  For example, $G_0$
...
$G_0$. Removing the back-edges of $L_0$ from $G_0$ (i.e., edges from $B_0$ to $H_0$) yields $G_1$, whose (only) SCC determines a further inner
loop $L_1 = (\{w,x\},
\{w,x\})$. Removing the back-edges of $L_1$ from $G_1$ results in the
acyclic $G_2$, terminating the process.

 \begin{figure}
    \begin{center}
    %\includegraphics[scale=0.3]{RamalingamSteensgaardConstruction}
    \tikzfigure{RamalingamSteensgaardConstruction}
    \end{center}
...
Figure~\ref{sub:ramal-steens-overlayTree}
...
of the back-edges $w \to u$ and $x \to v$.

The loop nesting forest resulting from Steensgaard's contruction is shown in
Figure~\ref{sub:ramal-steens-overlayLNF}.
Loops are drawn as ellipsis decorated with the appropriate header 
nodes, and nested in accordance with the containment relation $B_1
\subset B_0$ between the bodies.
  ...
  \begin{figure}
    ...
    \hfill
    \subfloat[]{
      \tikzsubfigure[1]{RamalingamSteensgaardOverlayAndLNF}
      \label{sub:ramal-steens-overlayTree}
    }
    \hfill
    \subfloat[]{
      \tikzsubfigure[2]{RamalingamSteensgaardOverlayAndLNF}
      \label{sub:ramal-steens-overlayLNF}
    }
    \hfill\null
    \end{center}
    \caption{Illustration of Steensgaard's construction of loop nesting forests: 
       \protect\subref{sub:ramal-steens-overlayTree} CFG-enriched dominance tree;
       \protect\subref{sub:ramal-steens-overlayLNF} resulting loop nesting forest.}
    \label{FigLoopAnalysisRamalingamSteensgaardOverlayAndANF} 
  \end{figure} 

  In the functional representation, a loop $L=(B,\{h_1,\ldots,h_n\})$ yields a function declaration block for
  ...
  accordance with the loop nesting forest.
  \begin{functional}
    \label{FunctionalSteensgaard}
     \begin{array}{l}
       \mathtt{function}\ \entrynode (\ldots)=\\ 
       \quad \mathtt{let} \funbody{\mathit{body\ of\ \entrynode}}\\
       \quad \mathtt{in}\ 
         \begin{array}[t]{l}
           \mathtt{letrec}\ (u,v) =
           \funcomment[13mm]{\mathit{define\ outer\ loop}\ L_0,\ \mathit{with\ headers}\ u, v}\\
           \quad \begin{array}[t]{l}
                     \mathtt{letrec}\ (w,x) =
                     \funcomment[9mm]{\mathit{define\ inner\ loop}\ L_1,\ \mathit{with\ headers}\ w, x}\\
                     \qquad 
                     \begin{array}{l}
                        \mathtt{let}\ \exitnode = 
                        \lambda p_{\mathit{exit}}.\ \ldots <\ \mathit{body\ of\ \exitnode} >\\
                        \mathtt{in}\ 
                          \begin{array}[t]{l}
                          (%(*function\ w*)\\
                           \ \lambda p_w.\ \funbody{\mathit{body\ of}\ w,\ 
                                            \mathit{with\ calls\ to}\ u,\ x,\
                                            \mathit{and}\ \exitnode}\\ 
                           % \ (*function\ x*)\\ 
                           \ ,\lambda p_x.\ \funbody{\mathit{body\ of}\ 
                                            x,\ \mathit{with\ calls\ to}\ w,\
                                            v,\ \mathit{and}\ \exitnode}\ )
                           \end{array}\\
                        \mathtt{end}\ \funcomment[28mm]{\mathit{end\ of\ inner\ loop}}
                     \end{array}\\
                     \mathtt{in}\ 
                     \begin{array}[t]{l}
                       (%(*function\ u}\
                        \  \lambda p_u.\ \funbody{\mathit{body\ of}\ u,\ 
                                     \mathit{with\ call\ to}\ w}\\ 
                        % \ (*function\ v}\
                        \ , \lambda p_v.\ \funbody{\mathit{body\ of}\ v,\ 
                                            \mathit{with\ call\ to}\ 
                                               x}\ )
                      \end{array}\\
                      \mathtt{end}\ \funcomment[34.3mm]{\mathit{end\ of\ outer\ loop}}
                    \end{array}\\
                    \mathtt{in}\ \funbody{\mathit{calls\ from}\ \entrynode\
                          \mathit{to}\ u\ \mathit{and}\ v}
         \end{array}
       \end{array}
  \end{functional}%
  ...
  the functional reading of the loop nesting forest extends the earlier correspondence
  ...
  several loops, as a CFG may contain several maximal SCCs. As the
  bodies of these SCCs are necessarily non-overlapping, the
  construction yields a forest comprised of trees shaped like the loop 
  nesting forest in Figure~\ref{sub:ramal-steens-overlayLNF}.
...
and Wads\-worth~\cite{Wadsworth00}. Early studies of CPS and direct
style include work by Reynolds and Plotkin~\cite{Reynolds:1972,Reynolds1974,Plotkin75}.  Two
...
efficient compilation to machine code,
 in particular with respect to their integration with program
analyses and optimizing transformations 
...
its logical conclusion. By mandating \emph{all} control-flow
points to be explicitly named, a uniform representation is obtained that
allows optimizing transformations to be implemented efficiently,
...
\emph{monadic} \index{monadic IR}intermediate languages as used by
Benton et al.~\cite{BentonKennedyRussel:ICFP1998} and Peyton-Jones et
al.~\cite{PeytonJonesShieldsLT:POPL1998}. These partition expressions
into categories of \emph{values} and \emph{computations}, similar to
the isolation of primitive terms in let-normal
form~\cite{Reynolds1974,Plotkin75}. This allows one to treat
side-effects (memory access, IO, exceptions, etc.) in a uniform way,
following Moggi~\cite{Moggi1991}, and thus simplifies reasoning about
program analyses and the associated transformations in the presence of
...
correctly introducing $\phi$-compensating assignments. The local algorithm to 
avoid the lost-copy problem and swap problem identified by Briggs et 
al.~\cite{BriggsSSA} was given by 
Beringer~\cite{DBLP:journals/entcs/Beringer07}. In this solution, the 
algorithm to break the cycles is in line with the results 
of May~\cite{May89}.
...
functional world that correspond to loop nesting forests.  Our discussion of
Steensgaard's construction was based on a classification of loop nesting forests by
Ramalingam~\cite{Ramalingam:2002:TOPLAS}, which also served
...
Sreedhar, Gao and Lee~\cite{Sreedhar:1996:TOPLAS}, and
Havlak~\cite{Havlak:1997:TOPLAS}.  To us, it appears that a
...
algorithm~\cite{WZ91}.  Beringer et
...
a simple imperative language rather than SSA. 

At present, intermediate languages in functional compilers do not
provide syntatic support for expressing nesting forest directly.
Indeed, most functional compilers do not perform advanced analyses of
nested loops. As an exception to this rule, the MLton compiler
(\verb|http://mlton.org|) implements Steensgaard's algorithm for
detecting loop nesting forests, leading to a subsequent analysis of
the loop unrolling and loop switching transformations~\cite{SurawskiMSc2016}.


%The textbook~\cite{DBLP:books/daglib/0015430} presents a unifying
%perspective on various program analysis techniques, including
%data-flow analysis, abstract interpretation, and type systems. As outlined
%in this book, the extension of type systems to effect systems
...
%program analysis, see Palsberg's
%article~\cite{Palsberg:2001:TAA:379605.379635}.
%
...
%include~\cite{winskel-93-formal,GunterBook,PierceTAPL}.
...
program point with types in the standard fashion, but the non-standard
...
Finally, Pop et al.'s model~\cite{PopJS2007} dispenses with control flow entirely and instead views programs as sets of equations that
...
%$\supseteq$ in the data-flow equations. Using inequations rather than
...
%from data-flow analysis frameworks.
%\begin{table}
%\begin{center}
%\begin{tabular}{|c|c|}
...
%  type systems & data-flow frameworks\\
%  typing context & scope-aware symbol table\\
%  typing
%  rules & data-flow (in-)equations/transfer functions\\
%  subtype relationship & merge operator\\
%  typing
%  type inference & fixed point iteration\\
%  derivations & solutions to data-flow equations\\
...
%definition.
%
%A major benefit of SSA for data-flow analyses is the avoidance of
...
%
%The correspondent to fixed point algorithms for obtaining data-flow
...
%Finally, \emph{solutions} of data-flow analyses arise when
...
%\section{Data-flow interpretations}
%\label{section:Part1:Semantics:Data-flow}
%The second group of interpretations for SSA is formed by models that
%emphasize data-flow aspects, i.e., the flow of values along the
%def-use-chains. Control flow is either disregarded entirely or
%restricted to the boundaries between basic blocks.

%\subsection{Glesner's intra-block data-flow model}
...
%the existence of values in the result slots of the data-flow
...
%emphasizes data-flow aspects, i.e., the flow of values along the
%def-use-chains.
...
%sequence like $x := 5;\ y:=x+z;\ x:=y\times 3$ may, for example, be
...
%non-termination in denotational semantics~\cite{winskel-93-formal}.
%
...
%\item [compatibility with the control-flow successor relationship:] this 
