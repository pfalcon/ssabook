...
\emph{free}\index{variable!free}\index{free|see{variable}}.  A well-formed 
...
%precise way to
\emph{continuation-passing-style}\index{continuation}\index{CPS|see{continuation}} 
(CPS), a program
...
desired minimal\todo{check minimality} code with a single $\phi$-function, for variable $y$
...
individually and avoids the ``lost-copy'' and ``swap'' problems (cf.~Chapter~\ref{alternative_ssa_destruction_algorithm})\index{lost-copy problem}\index{swap problem}:
...
In the case of a \emph{reducible}\index{reducible} CFG, the resulting graph contains
...
In the case of \emph{irreducible}\index{irreducible} CFGs, the enriched dominance tree
is no longer acyclic (even when ignoring self-loops). In this case,
...
different partitioning are possible, corresponding to different
notions of \emph{loop nesting forests}\index{loop nesting forest}. Of 
the various loop nesting forest
...
\emph{monadic} \index{monadic}intermediate languages as used by Benton et
...
let-normal form~\cite{Reynolds1974,Plotkin75}. This allows
one to treat side-effects (memory access, IO, exceptions, etc.) in a
