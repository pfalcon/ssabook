...
%  \fbox{$
%   \begin{array}[t]{l} 
%    \texttt{let}\ y = (\texttt{let}\ y=2*v \ \texttt{in}\ \fbox{$4*y$}_{v,y}\ \texttt{end})\\
%    \texttt{in}\ \fbox{$y*v$}_{v,y}\ \texttt{end}\\
%\end{array}$}_v\\
%\texttt{end}
%\end{array}
...
%$\texttt{b_1:}$ & $\texttt{v:=1}$ & \quad &
%  $\texttt{b_2:}$ & $\texttt{x:=5+y}$ & \quad &
%  $\texttt{b_3:}$ & $\texttt{w:=y+v}$\\
%& $\texttt{z:=8}$ & & & $\texttt{y=y*z}$ & & & $\texttt{return\ w}$\\
%& $\texttt{y:=4}$ & & & $\texttt{x:=x-1}$\\
%& $\texttt{goto}\ \texttt{b_2}$ & & & $\texttt{if\ x=0\ b_3\ b_2}$
%\end{tabular}
...
%                     {\call {f_2} {v,z,y}}\ \texttt{end}\ \texttt{end}\ \texttt{end}
%              \end{array}\\
%             \texttt{end}
%           \end{array}\\
%     \, \texttt{in}\ \letin v 1 {
...
%\mathsf{Use}(a) \cup (\LV(\textit{succ}(i)) \setminus \mathsf{Defs}(i))$$ 
...
%left, i.e., when $e$ evaluates for the first time to $\texttt{false}$.
%
%For example, the representation of
%$\texttt{i=7;\ j=0;\ while\ (j<10)\ \{j=j+i\}}$
