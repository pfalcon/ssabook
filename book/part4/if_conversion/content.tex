...
\begin{figure}
  ...
  \includegraphics[scale=0.9]{phi_augmentation}
  \label{fig:phi_aug}}
...
The algorithm creates a list of candidate block having 2 exit edges in postorder traversal of the control flow. Then it iterates on this list. Since inner tests will appear first in the postorder list, the if-conversion will process the regions for inner to outer, and for each iteration will reconsider the cost of if-conversion. 
...
Consider the control flow transformations for the famous $wc$ program (figure \ref{fig:wc1}).
