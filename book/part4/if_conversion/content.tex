...
\index{if-conversion}
The if-conversion optimization presented in Chapter~\ref{chapter:ssa-codegen} can easily been expressed in SSA thanks to a \texttt{select} or $\psi$ representation of the conditional instructions.
...
Each basic block in the list designates the head of a sub-graph that can be if-converted using the transformations described in Section~\ref{sec:basic}. Post-order traversal allows to process each region from inner to outer. When the if-converted region cannot grow anymore because of resources, or because a basic block cannot be if-converted, then the next sub-graph candidate is considered until all the CFG is explored.
%
...
%
Note that as the reduction proceeds, maintaining SSA\index{incremental update, of SSA} can be done using the general technique described in Chapter~\ref{chapter:repair_maintain_ssa_after_optimization}, or basic ad-hoc updates can be implemented instead locally. 

Consider for example the CFG in Figure~\ref{fig:wc1}. The exit node $\textit{BB}7$, and basic block $\textit{BB}3$ that contains a function call cannot be if-converted (represented in gray) and are excluded from the critical path and the considered region can be if-converted into an hyperblock.

\begin{figure}
  \subfloat[Initial] {
    \includegraphics[scale=0.6]{graph_a}
    \label{fig:wc1}}
  \subfloat[If-converted] {
    \hspace{1em}\includegraphics[scale=0.6]{graph_b}\hspace{1em}
    \label{fig:wc2}}
\caption{\label{fig:wc example}If-conversion of \texttt{wc} (word count program). basic blocks in gray cannot be if-converted.}
\end{figure}

 The post-order list of conditional blocks (represented in bold) is [$\textit{BB}9$, $\textit{BB}14$, $\textit{BB}13$, $\textit{BB}11$, $\textit{BB}8$, $\textit{BB}7$, $\textit{BB}5$, $\textit{BB}2$]. 
The first candidate region is composed of $\textit{BB}9$, $\textit{BB}10$, $\textit{BB}2$. $\phi$ reduction can be applied, promoting the instructions of $\textit{BB}10$ in $\textit{BB}9$. $\textit{BB}2$ becomes the single successor of $\textit{BB}9$. 
%
The region headed by $\textit{BB}14$ is then considered. $\textit{BB}15$ cannot yet be promoted because of the multiple side entries. Thus $\textit{BB}15$ is duplicated into a $\textit{BB}15'$ and can then be promoted into $\textit{BB}14$.
%
The region headed by $\textit{BB}13$ that have now $\textit{BB}17$ and $\textit{BB}15$ as successors is considered. $\textit{BB}15$ is duplicated into $\textit{BB}15'$, so as to promote $\textit{BB}17$ and $\textit{BB}15'$ into $\textit{BB}16$ through $\phi$ reduction. Note that $\textit{BB}15'$ already contains predicated operations from the previous transformation. So, a new merging predicate is computed and inserted. After the $\phi$ removal is fully completed $\textit{BB}16$ has a unique successor, $\textit{BB}2$.
%
$\textit{BB}11$ is the head of the new candidate region where $\textit{BB}15$, $\textit{BB}16$ can be promoted. Again since $\textit{BB}16$ contains predicated and predicate setting operations, a newly predicate must be created to hold the merged conditions.
$\textit{BB}10$ is then considered; $\textit{BB}11$ needs to be duplicated to $\textit{BB}11'$. The process finished with the region head by $\textit{BB}7$.

\subsection{Tail Duplication}

\index{tail duplication}
Just as for the example of Figure~\ref{fig:wc example}, some basic blocks (such as \textit{BB3}) may have to be excluded from the region to if-convert. \emph{Tail duplication} can be used for this purpose. Similar to path duplication\index{path duplication} described in Section~\ref{sec:basic}, the goal of tail duplication is to get rid of incoming edges of a region to schedule. This is usually done in the context of hyperblock formation\index{hyperblock formation}, which technique consists in, as opposed to the inner-outer incremental technique described in this chapter, to if-convert a region in ``one shot''. Consider again the example of Figure~\ref{fig:wc2}, getting rid of the incoming edge from \textit{BB3} is possible by duplicating all basic blocks of the region reachable from \textit{BB5} as shown in Figure~\ref{fig:wc3}. The duplicated \textit{BB5} can now be scheduled with respectively \textit{BB3} and \textit{BB4}.

\begin{figure}
    \subfloat[Tail duplication] {
    \includegraphics[scale=0.6]{graph_d}
    \label{fig:wc3}}
\caption{\label{fig:tail example}Tail duplication can be used to exclude \textit{BB3} from the selected region.}
\end{figure}
...
The duality between SSA like $\phi$s and predicate dependencies have been used in other works. In SSA-PS \cite{Jacome01clusteredvliw}, Predicated Switching operations are used to realize the conditional assignments using aggressive speculation techniques with conditional moves. Phi-Predication \cite{Chuang03phi-predicationfor} uses a modified version of the RK algorithm, to map phi-predication with phi-lists, holding guard and topological information. 
