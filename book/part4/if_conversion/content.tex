...
For all transformations but the conjunctive predicate merging, there are two such paths starting at basic-block \textit{head}. For the code of Figure~\ref{fig:paths_reduction}, we would have $\textit{path}_p=[\textit{head}, B_1, \textit{exit}[$ and $\textit{path}_{\overline{p}}=[\textit{head}, B'_1, B'_2, \textit{exit}[$ of respective probability $\proba{p}$ and $\proba{\overline{p}}$. For a path $\textit{P}_q=[B_0, B_1, \dots, B_n[$ of probability $\proba{q}$, its cost is given by 
$\cost{\textit{P}_q}=\proba{q}\times \sum_{i=0}^{n-1} \left(\cost{B_i}+\cost{(B_i,B_{i+1})}\right)$ where the cost a basic-block $\cost{B_i}$ is estimated using its schedule height; the cost of an edge $\cost{(B_i,B_{i+1})}$ is set to the branch latency $\brlat$ if it corresponds to a taken conditional branch or to 0 otherwise.

Let $\textit{path}_p=[\textit{head}, B_1, \dots, B_n, \textit{exit}[$ and $\textit{path}_{\overline{p}}=[\textit{head}, B'_1, \dots, B'_m, \textit{exit}[$ be the two paths. 
Then $\cost{\textit{path}_p}+\cost{\textit{path}_{\overline{p}}}$, the overall cost  before if-conversion  simplifies to
$$\textsf{cost}_{\textsf{control}}=\cost{\textit{head}} + \proba{p}\times\left(\brlat+\sum_{i=0}^n \cost{B_i}\right) + \proba{\overline{p}}\times\sum_{i=0}^m \cost{B'_i}$$
This is to be compared to the cost after if-conversion
$$\textsf{cost}_{\textsf{predicated}}=\cost{\textit{head} \circ \left(\bigcirc_{i=1}^n B_i\right)\circ \left(\bigcirc_{i=1}^m B'_i\right)  }$$
...
The profitability for the logical disjunctive and conjunctive merge (convergent or not)  can be done similarly. As shown in Figure~\ref{fig:paths_convergent} there are three paths impacted by the transformation: $\textit{path}_{p\wedge q}=[\textit{head}, \textit{side}, B_1[$, $\textit{path}_{p\wedge \overline{q}}=[\textit{head}, \textit{side}, \textit{exit}[$, and $\textit{path}_{\overline{p}}=[\textit{head}, \textit{exit}[$ of respective probability  $\proba{p\wedge q}$, $\proba{p\wedge \overline{q}}$, and $\proba{\overline{p}}$. The overall cost before the transformation $\cost{\textit{path}_{p\wedge q}}+\cost{\textit{path}_{p\wedge \overline{q}}}+\cost{\textit{path}_{\overline{p}}}$ simplifies to
$$\textsf{cost}_{\textsf{control}}=\cost{\textit{head}}+(1+\proba{p})\times\brlat$$
which should be compared to
$$\textsf{cost}_{\textsf{predicated}}=\cost{\textit{head}\circ \textit{side}}+\brlat$$
...
Note that if $\proba{p}\ll 1$, emitting a conjunctive merge might not be beneficial. In that case, another strategy such as path duplication from the \textit{exit} block will be evaluated.
