...
Merging multiple control flow paths together can easily exceed processors resources, leading to excessive register pressure or move infrequently used long latencies instructions into the critical path. 

The local decision function to if-convert, or not a sub-region in order to enlarge the hyperblock is based on constraints and heuristic.
Constraint based if-conversion selection algorithm will consider differently predicative instructions, that cannot be executed unconditionally, and speculative instructions, that merge into conditional moves; or trapping instructions that will prevent the region to be if-converted. This constraint is evaluated at instruction level, so a basic block can contain a mix of speculative and predicative instructions
Heuristic based if-conversion selection is performed after the executions are known to be constrained on predication or speculation.

The objective function used in this heuristic can then include the live predicate and an approximation on registe pressure as well as the operations introduced by the if-conversion process itself. Such operations include predicate merging operations, $\psi$ operations that can sefely be accounted as two moves, or $select$ operations.

This function should be aggressive enough to maximize the use of resources, but without over-committing them, because of the risk of spilling registers or bad schedule if multiple instruction dependent of a scare resource. e.g a multiplier, a data bus or a floating point unit. Lead to conservative heuristics, or reverse if-conversion.
...
This heuristic can only evaluates the register pressure in the case of speculation, because the variables now interfere. Computing the interference graph and simulating a register coalescing process would be only account for local effects, and would lack precision. Despite this, experience showed that it might be more easy to deal with in the SSA framework because of the locality of the region under consideration.
