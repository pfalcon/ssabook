...
\index{if-conversion} Very Large Instruction Word (VLIW)\index{Very Long Instruction Word, VLIW} or Explicitly Parallel Instruction Computing (EPIC) architectures make Instruction Level Parallelism (ILP)\index{Instruction Level Parallelism, ILP} visible within the Instruction Set Architecture (ISA), relying on static schedulers\index{scheduling} to organize the compiler output such that multiple instructions can be issued in each cycle.

If-conversion is the process of transforming a control-flow region with conditional branches, into an equivalent predicated\index{predicated execution} or speculated\index{speculative execution} sequence of instructions (into a region of Basic Blocks (possible single) referred to as an Hyperblock. 
...
The \textit{merge} pseudo operations need to be mapped to a conditional form of execution in the target's architecture. As illustrated by Figure~\ref{fig:pred} we differentiate the three following models of conditional execution:
\begin{itemize}
...
SSA form solves the renaming issue for free, and as illustrated by Figure~\ref{fig:pred_min} the minimality\index{minimal SSA form}\index{pruned SSA form} and the pruned flavor of the SSA form allows to avoid inserting useless {select} operations.
...
On the other hand, on register allocated code, speculation adds anti-dependencies\index{dependence, anti-}. 
...
The only subtlety related to projection is related to generating the new predicate as the logical conjunction of the original guard (e.g., $\overline{p}$)\index{guard} and the current branch predicate (e.g., $q$). 
...
Consider for example the CFG from the gnu \texttt{wc} (word count) program reported in Figure~\ref{fig:wc1}. 
...
This is usually done in the context of hyperblock formation\index{hyperblock formation}, which technique consists in, as opposed to the inner-outer incremental technique described in this chapter, to if-convert a region in ``one shot''. 
