...
In the following, we describe the modeling of code selection for SSA
...
We will now outline a possible problem formulation for these
generalized patterns in the PBQP framework discussed so far. To do so,
we must ensure that \emph{a)} we end up with a valid graph cover and
\emph{b)} there exists a topological order that respects the partial
order defined by edges in the SSA graph. As the latter requirement is
less obvious, we illustrate it with an example in
...
from the edges shown in the SSA graph. The example gives rise to a
topological order as long as we do not select \emph{all} three
instances of the post-increment store pattern concurrently. The
existence of a topological order is a global property, which has
to be considered in the modeling of the problem.
...
\begin{algorithm}
  ...
  \ForAll{basic blocks $b$} {
    compute a topological order for the subgraph induced by $b$\;
    apply semantic rules associated with the chosen productions\;
  }
  \caption{\label{alg:pbqpisel}Generalized PBQP instruction selection}
\end{algorithm}
...
proper order on the subgraphs induced by individual basic blocks.
...
postincrement store pattern (surrounded by boxes) in the example shown
...
and to zero for the \textit{off} state. Furthermore, the domain of
...
including the node. The local costs for these proxy states are set to
zero.
...
addition to the postincrement store pattern with costs 3, we assume
regular tree patterns for the store and the increment nodes with costs
two denoted by \texttt{P2} and \texttt{P3} respectively. Rules for the
...
pattern instance (which can be modeled with combined costs of zero or
...
nonterminal symbols involved.  It should be noted that for general
...
ensures a correct but possibly suboptimal matching. These limitations
...
Instruction selection on SSA-graph enables aggressive code selection
optimizations. The whole flow of a function is taken into account
...
are often sub-optimal. In~\cite{1269857}, a polynomial-time algorithm
