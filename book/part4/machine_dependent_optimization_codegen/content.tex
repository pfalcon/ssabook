...
Predicated or conditional execution is the other main case of definitions that do
not kill the target register. Predicated and conditional execution refer to:
\begin{description}

\item[\bf Partial predicated execution support] Availability of SELECT
instructions like those of the Multiflow TRACE architecture
\cite{Colwell:1987:ASPLOS}. The Multiflow TRACE 500 architecture was to include
predicated store and floating-point instructions \cite{Lowney:1993:JS}.
...
conditions, including Unconditional, OR-style, AND-style PDIs
\cite{Mahlke:1995:ISCA}.

\item[\bf Partial conditional execution support] Availability of conditional
move (CMOV) instructions similar to those introduced in the Alpha AXP
architecture \cite{Blickstein:1992:DTJ}. In particular CMOV instructions are
available in the ia32 ISA since the Pentium Pro.
...
presented in Chapter~\ref{chap:psi_ssa} specifically address the handling
operations that do not kill the target operand because of predicated or
conditional execution.

Observe however that under the SSA form, a CMOV instruction is
equivalent to a SELECT instruction with a 'must be same register' naming
constraint between one source and the target operand. Unlike other predicated or
conditional instructions, the SELECT instructions kill the target register.
Generalizing this observation provides a simple way to handle predicated
instruction sets in vanilla SSA form: \begin{itemize}
...
execution \cite{Mahlke:1995:ISCA}:
In principle, any if-conversion technique targeted to full
...
reversible side effects, so speculating potentially excepting instructions
...
memory loads known as 'dismissible' are provided. Further architectural support
...
predicated PDIs. The R function assigns a minimal set of Boolean predicates to
...
the Multiflow compiler in order to generate SELECT operations and predicate
memory store operations.

\item Fang \cite{Fang:1996:LCPC} assumes a fully predicated architecture with
predicated PDIs. The proposed algorithm is tailored to acyclic regions with
...
\end{itemize} A few contributions to if-conversion only rely on the SSA form
$\phi$-placement: \begin{itemize}

\item Jacome et al. {Jacome:2001:DAC} propose the Static Single Assignment --
Predicated Switching transformation (SSA-PS) technique aimed at clustered VLIW
architectures, with predicated move instructions that operate inside clusters
(internal moves) or between clusters (external moves). The main idea is to
realize the conditional assignments corresponding to $\phi$-functions by
using predicated move operations. The predicated external moves leverage the
penalties associated with inter-cluster data transfers.
...
%$\psi$-functions in order to handle predicated code under the SSA form.
%The $\psi$-functions arguments are paired with predicates and are
%ordered in dominance order.
...
\item Bruel \cite{Bruel:2006:ODES} revisits the Multiflow compiler if-conversion
algorithms by reducing acyclic control-flow constructs from innermost to
outermost, intermingled with tail duplication. This technique operates on the
SSA form and generates $\psi$-functions for predicating the memory operations of
the ST200 / Lx VLIW architecture family. A generalization is described in
Chapter~\ref{chap:if_conv}.

\item Ferri\`ere \cite{Ferriere:2007:SCOPES} TBD
...
Induction variable classification (Chapter~\ref{chap:}),
