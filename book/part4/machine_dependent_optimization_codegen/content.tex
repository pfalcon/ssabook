...
dismissible memory load instructions. The proposed technique reduces acyclic
control-flow constructs from innermost to outermost, and the monitoring of the
if-conversion benefits provides the stopping criterion. This technique control
speculates operations, reduces height of predicate computations, and perform
tail duplication. A generalization is described in Chapter~\ref{chap:if_conv}
which generates $\psi$-functions

\item Ferri\`ere \cite{Ferriere:2007:SCOPES} generalizes 

\end{itemize}

Thanks to the $\psi$-SSA representation, if-converted code can be represented
under the SSA form, and if-conversion algorithms can operate directly on the SSA
from. Virtually all if-conversion techniques formulated before the $\psi$-SSA
representation can be adapted to it, so only two main choices of a particular
technique
