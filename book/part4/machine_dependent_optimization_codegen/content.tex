...
provided by conditional move of SELECT instructions \cite{Mahlke:1995:ISCA}.
The former are predicated, while the latter always write to their destination
register. Observe that under the SSA form, a conditional move instruction is
equivalent to a SELECT instruction with a 'must be same register' naming constraint
between one source and the target operand. Generalizing this observation
provides a simple way to handle predicated instructions: \begin{itemize}
...
% Mainly for use by instruction scheduling:
% - remove branch penalty
% - lower use of the branch unit
% - increase the size of scheduling regions
% Fully resolved predicates, partially resolved predicates
