...
generator\index{code generation} covers the set of code transformations and optimizations that operate
on a program representation close to the target machine Instruction Set
Architecture (ISA)\index{instruction set architecture!ISA}. The code generator produces an
...
Representation (IR)~\cite{Stanier:2013:CS} to the target machine instructions and calling conventions\index{calling conventions};
...
\begin{itemize}
\item Instruction selection\index{code selection} and lowering of the calling conventions.
\item Control-flow (dominators, loops) and data-flow (variable liveness\index{liveness}) analyses.
\item Register allocation\index{register allocation} and stack frame building.
...
\begin{itemize}
\item If-conversion using \texttt{select}\index{select, instruction}, conditional move, or predicated, instructions (Chapter~\ref{chapter:if_conversion}).
\item Use of specialized addressing modes\index{two-address mode} such as auto-modified addressing~\cite{Lee:2003:TODAES} and modulo
addressing.
\item Exploitation of hardware looping\index{hardware looping}~\cite{Leupers:1997:Book} or static branch prediction hints.
\item Matching fixed-point arithmetic and SIMD\index{SIMD} idioms to special instructions.
\item Memory hierarchy optimizations, including cache prefetching and register preloading~\cite{Dinechin:1999:CC}.
\item VLIW\index{VLIW} instruction bundling, where parallel instruction groups constructed by postpass
...
assignment problem (PBQP).\fi
\ifregin
Although there is a debate as to whether or not SSA
form should be used in a register allocator,
Chapter~\ref{chapter:register_allocation} makes a convincing case for it.\fi
The
...
assembly optimizers for the ST120 DSP core~\cite{Dinechin:2000:ME}\cite{Dinechin:2000:CASES,Stoutchinin:2001:MICRO,Rastello:2004:CGO} and the
Lx/ST200 VLIW family~\cite{Faraboschi:2000:ISCA}\cite{Dinechin:2007:MISTA,Dinechin:2008:EuroPar,Boissinot:2008:CGO,Boissinot:2009:CGO,Boissinot:2011:APLAS}.
...
does not seem to benefit from the SSA form.  Constructing and destructing SSA form in a
...
(ISA)\index{instruction set architecture!ISA|mainidx}. Instructions access values and modify the machine state through
\emph{operand}s. We distinguish \emph{explicit operands}\index{operands, explicit|mainidx}, which are associated
with a specific bit-field in the instruction encoding, from \emph{implicit
operands}\index{operands, implicit|mainidx}, without any encoding bits.  Explicit operands correspond to
allocatable architectural registers, immediate values, or instruction modifiers.
Implicit operands correspond to dedicated architectural registers\index{dedicated registers|mainidx},
...
An \emph{operation}\index{operation|mainidx} is an instance of an instruction\index{instruction|mainidx} that composes a program. It
is seen by the compiler as an \emph{operator}\index{operator|mainidx} applied to a list of operands
...
\emph{indirect operands}\index{operand, indirect|mainidx}, which are not apparent in the instruction behavior,
...
such as constant propagation\index{constant propagation} or sign extension removal need to know what is
...
combined division-modulus instructions, or side-effects on status registers\index{registers, status}.

There are two ways to associate machine instructions with semantics: \begin{itemize}
...
appears. For instance, range propagation\index{range analysis} may ensure that an addition cannot
overflow, that a division by zero is impossible\index{data-flow analysis, non-zero value}, or that a memory access is
safe for control speculation\index{speculation} . Context-dependent semantics, which needs to be
...
pseudo-instructions\index{instruction, pseudo-} with standard semantics be available, besides variants of
\phifuns and parallel copy\index{parallel copy} operations. \begin{itemize}

\item Machine instructions that operate on register pairs, such as the long
multiplies on the ARM, or more generally on register tuples\index{register tuples}, must be handled. In such
...
loops\index{hardware loop|mainidx}), where an implicit conditional branch back to the loop header is taken
...
\item Register allocation\index{register allocation} for predicated architectures requires that the live-ranges
of temporary variables with predicated definitions be contained by
pseudo-instructions~\cite{Gillies:1996:MICRO} that provide backward kill points\index{instruction, kill} for liveness analysis.
...
Explicit instruction operands may be constrained to use the same resource\index{pinning} (an
...
resource}\index{two-address mode} naming constraints are represented under the SSA form by inserting a
...
constrained source operands, a parallel copy\index{parallel copy} operation is used.  Again, those
...
specific architectural register at all points in the program\index{live range pinning}. This is the case
...
Predicated execution\index{predicated execution} and conditional execution are other sources of definitions
...
\item[\textbf{Partial predicated execution support}] \texttt{select} instructions\index{select, instruction}, first
...
evaluates to \false. EPIC-style architectures\index{EPIC} also provide predicate define
instructions (PDIs)\index{instruction, predicate define} to efficiently evaluate predicates corresponding to nested
...
instructions\index{cmov, instruction}, first introduced by the Alpha AXP architecture~\cite{Blickstein:1992:DTJ}, are provided. \index{cmov} instructions are available in the
...
%conditional) the instruction set architectures that include \texttt{select} (CMOV)
...
transient\index{transient information}, or belongs to the invariants of the program representation.  An
\emph{invariant}\index{invariants} is a property which is ensured before and after each phase.
...
\phifuns\index{\phifun, as multiplexer}, \sigmafuns\index{\sigmafun} of the SSI form~\cite{Boissinot:2011:APLAS} (see Chapter~\ref{chapter:ssi}), and
parallel copies\index{parallel copy} may appear as regular operations constrained to specific places
...
construction of a loop nesting forest\index{loop nesting forest} as defined by Ramalingam~\cite{Ramalingam:2002:TOPLAS}. Non-reducible control-flow allows for different
...
\emph{conventional}\index{conventional SSA} across the code generation phases. This approach is
motivated by the fact that classic optimizations such as SSAPRE\index{SSAPRE}~\cite{Kennedy:1999:TOPLAS} (see Chapter~\ref{chapter:pre_not_helped}) require that 'the live ranges of different versions
...
et al.~\cite{Boissinot:2009:CGO} clarified how to convert the transformed SSA\index{transformed SSA, T-SSA}
...
If-conversion\index{if-conversion} refers to optimizations that convert a program region to
...
speculating potentially excepting instructions\index{excepting instruction} requires architectural support.
...
\emph{dismissible}\index{load, dismissible} are provided. The IMPACT EPIC architecture speculative
...
Boolean guards accordingly. As this work pre-dates the Program Dependence Graph\index{program dependence graph}~\cite{Ferrante:1987:TOPLAS} (see Chapter~\ref{chapter:vsdg}), complexity of the resulting Boolean expressions is
...
algorithm' based on control dependences\index{dependence, control-}. They assume a fully predicated
...
\texttt{select} and the predicated memory store\index{store, predicated-} operations.
...
promotion}\index{predicate promotion}\footnote{The predicate used to guard an operation is promoted to a
weaker condition},
...
reduction of worst-case execution time (WCET)\index{WCET}.
...
Single Assignment - Predicated Switching (SSA-PS)\index{predicated switching, SSA-} transformation. It assumes a
...
assignments corresponding to \phifuns via predicated switching\index{predicated switching instruction}
...
They propose \texttt{select} instructions called \emph{phi-ops}\index{phi-ops}, predicated memory
...
regions\index{single-entry-single-exit region}, and adapted to the proposed architectural support.
...
\phifuns source operands are replaced by \emph{phi-lists}\index{phi-lists}, where each
...
\end{itemize}
The result of an if-converted region is a hyper-block\index{hyper-block|mainidx}, that is, a sequence of
...
%computed like the placement of the \phifuns. However the source operands
%of \phifuns are replaced by so-called $\phi$-lists, where each operand
...
is prepass instruction scheduling\index{instruction scheduling, prepass}. Innermost loops with a single basic block,
...
scheduling includes virtual register renaming\index{virtual registers|see{pseudo registers}}, also known as register web\index{web}
...
have control-flow merge\index{CFG merge}. So there are no \phifuns in case of acyclic
...
\item correct SSA form destruction is the removal of \phifuns from
...
\cite{Eichenberger:1995:MICRO,Johnson:1996:MICRO}
