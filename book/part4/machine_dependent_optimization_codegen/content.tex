...
did not consider the SSA form: \begin{itemize}

\item[Allen et al. \cite{Allen:1983:POPL}] convert control dependences to data
...
graphs, but in practice it is applied to single entry acyclic regions.
...
\item[Lowney et al. \cite{Lowney:1993:JS}] match the innermost if-then
...
if-then-else statements on architectures with full conditional execution
...
each if-then-else statement, and the objective is the reduction of worst-case
execution time (WCET).

\end{itemize}

A few contributions to if-conversion did use the SSA form
but only internally: \begin{itemize}

\item[Jacome et al. \cite{Jacome:2001:DAC}] propose the Static Single
Assignment - Predicated Switching (SSA-PS) transformation aimed at clustered
VLIW architectures fitted with predicated move instructions that operate inside
...
of predicated or conditional execution: \begin{itemize}
