...
\subsection{Motivation for the SSA form in code generation}
...
\item Analyzes such as induction variable classification, liveness sets, and
liveness checking (Chapter~\ref{chap:liveness_analysis}).
...
\item Cleanups including dead code elimination and instruction
re-selection.

\end{itemize}

Further down the code generator, the next major phase is pre-pass instruction
scheduling. On innermost loops whose bodies are single basic block or super
block, software pipelining techniques such as modulo scheduling may apply. On
innermost loops that are not software pipelined, and for the other program
regions, acyclic instruction scheduling techniques apply: basic block
...
pinning, computing interference.
