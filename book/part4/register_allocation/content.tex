...
return stack ;
\end{verbatim}
\caption{Greedy simplification scheme used by the greedy coloring scheme to create the stack.}
...
to each interference graph node is associated a unique register to which the associated variable will be allocated along its entire live-range.
...
the value of variable $c$ will be contained in $R_2$ and needs to transferred to $R_1$, which means the final code will contain an instruction ``{\tt mov $R_1, R_2$}'' or the like. 
...
This is possible if we are under conventional SSA (C-SSA, see Chapter~\ref{chapter:properties_and_flavours}), but optimizations can break this property and interferences can appear between subscripted variables with the same origin.
Such two variables cannot be assigned to the same register without breaking the program.

To bypass this problem and also allow for more involved coalescing between SSA variables of different origins, we define a notion of \emph{affinity}, acting as the converse of the relation of interference and expressing how much two variables ``want'' to share the same register. 
