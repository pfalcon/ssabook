...
First, it still is chordal, hence easily colorable in polynomial time (more details in Section~\ref{sec:greedy-col});
Second, we know that $R$~colors are sufficient to color it since $\maxlive \leq R$.
We could be ending the register allocation phase now, but we would be missing the second most important optimization after spilling: \emph{coalescing}.

The goal of coalescing is to minimize the number of register-to-register {\tt move} instructions in the final code.
...
We will first present the traditional graph coloring heuristic of Chaitin et al.\ and then show how it successfully color programs under SSA form.
We will then introduce the notion of \emph{affinities} between variables and how to use them to drive the coloring so as to remove as much copies as possible.

 

\subsection{Greedy coloring scheme and SSA form}
...
\label{sec:greedy-col}

In traditional graph coloring register allocation algorithms, the assignment is done by coloring the interference graph with the greedy heuristic of Chaitin et al.~\cite{Chaitin81}.
...
\subsubsection{SSA graphs are greedy.}
...
Let us consider such a representation (see Figure~\ref{fig:regalloc:subtree} TODO) and try to apply the greedy coloring scheme to it.
%This means we can simplify live-ranges which intersect with at most $R-1$ other live-ranges.
Let us also consider the ``leaf'' of a branch of the dominance tree, i.e., the live-range, on this branch which starts the last.
Suppose this leaf intersect with other live-ranges (if not, we can simplify it), then they all start before it and end after its start.
So, all neighbours of the leaf are live at its starting point (i.e., at the definition of the variable).
Since spilling has already been done, we know that the register pressure at this point is at most $R$;
The leaf has at most $R-1$ neighbours and can be simplified.
Removing the leaf does not change the shape of the graph: the remaining live-ranges are still subtrees of the dominance tree;
Hence, the process can continue and we are guaranteed that, as long at the graph is not empty, there will still be live-ranges that can be simplified.



Consider the variables alive at a leaf of the dominance 
...
An interesting fact is that the success or failure of this greedy scheme is independent of the order of simplification.
In other words, if the greedy scheme fails at some point, there is no simplification order that could simplify entirely the graph.
This is because the remaining nodes have degree greater than~$R$ at all time and there is no way to reduce their degree other than simplifying one of them, which is impossible.
The side effect is that the greedy scheme defines a class of graphs that we call the \emph{greedy-$k$-colorable} graphs.
These are the graphs $k$-colorable using the greedy scheme.
The pseudo-code for testing if a graph is greedy-$k$-colorable is given on Figure~\ref{code:is-k-greedy}.
...
In the case of SSA, it is for instance obviously better to assign variables linked by a \phifun, to the same register.
This is in fact true of all subscripts of the same variable ($a_1$, $a_2$, etc.).
In general we can define a notion of \emph{affinity}, acting as the converse of the relation of interference and expressing how much two variables ``want'' to share the same register. 
...
For instancfor two variables $a$ and $b$, count 1 for each time they appear in the same copy instruction ($a\gets b$ or $b\gets a$) or \phifun ($a\gets\phi(\ldots,b,\ldots)$ or $b\gets\phi(\ldots,a,\ldots)$). 
These numbers can of course be weighted using empirically parameters (classically, times ten for each level of nested loop, halved if in a conditional branch, etc.) or using actual execution frequencies based on profiling.
