...
However, it requires constructing and maintaining an interference graph, a structure judged to big and cumbersome to be used in a JIT context, where compilation time and memory prints matters more. 
...
The variables are considered in the order of their definitions; A pseudo-code is given in procedure Tree\_Scan, Figure~\ref{code:assign-tree-scan}.
Intuitively, this method works because when the scanning arrives at the definition of the variable, the only colored variables are ``above'' it and since there is at most $R-1$ other variables live at the definition, there is always a free color.
...
\begin{proof}
  ...
  if $v_i$ dominates $v_j$, $v_j$ appear before $v_i$ in the ordering: $j < i$.
  ...
  If $v_i$ dominates any variable, say $v_j$, then $i < j$ and the node has already been simplified;
  So, there is no variable defined after $v_i$ on the dominance tree.
  This means the live-range of $v_i$ is a ``leaf'' of the subtree representation;
  Based on observation in the previous section, this means $v_i$ can be chosen for simplification.
  Hence, the ordering $v_1, \ldots, v_n$ is a valid order of simplification and its reverse $v_n, v_{n-1}, \ldots, v_1$ is a valid order for coloring.
\end{proof}
...
In that case, coloring a branch would constrain the coloring on a different branch while under SSA form they are independent.
...
Tree_Scan(T)
  ...
  function assign_color(p, available)
    for each v last use at p
      available[color(c)] = True     /* colors not used anymore */
