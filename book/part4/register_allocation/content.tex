...
Further_First(B)
  function evict(p, &in_regs, &in_mem, in_solidstate)
    while |in_regs|>R
      ...
      if v not in in_mem 
         insert a store of v to memory just after its definition point
         in_mem = in_mem U {v}
      in_regs = in_regs \ {v}
...
\item assuming low register pressure within the loop, we have $x.\texttt{HP}(p_0)=\{p_0\}$, and $y.\texttt{HP}(p_0)=\{p_0,p_3\}$. 
  This leads to $\texttt{x.spill\_profitability}(p_0)=1$ and $\texttt{y.spill\_profitability}(p_0)=1+0.5=1.5$, ie. evicting $y$.
\item now assume register pressure to be high at $p_1$, then $x.\texttt{HP}(p_0)=\{p_0,p_1\}$, and $y.\texttt{HP}(p_0)=\{p_0,p_3\}$. This leads to $\texttt{x.spill\_profitability}(p_0)=1+50=51$ and $\texttt{y.spill\_profitability}(p_0)=1.5$, i.e. evicting $x$.
\end{enumerate}
