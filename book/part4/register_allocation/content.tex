...
\begin{figure}
  ...
  % \begin{center}
    ...
    \kern-2em
    \subfloat[Graph-based]{\tikzsubfigure[1]{running}%
      \label{sub:ra:running-gr}
    }
    \subfloat[Interference graph]{%
      \quad
      \tikzfigure{running-ig}%
...
\begin{algorithm}
  ...
  \Fn{assign\_color ($p$, available)}{
    ...
    \ForEach{$v$ last use at $p$}{
      available[color($v$)] $\gets$ \KwTrue\tcc*{colors not used anymore} \label{ra:linecode:assign-tree-scan:clear}
    }
    \ForEach{$v$ defined at $p$}{
      $c \gets$ choose\_color($v$, available)\tcc*{pick one of the available colors} \label{ra:linecode:assign-tree-scan:assign}
...
return stack ;
\end{verbatim}
\caption{Greedy simplification scheme used by the greedy coloring scheme to create the stack. If this stage succeeds, the graph is said to be \gr{k}.}
...
When a graph can be simplified using the function described in Figure~\ref{ra:code:is-k-greedy}, we say that this graph is \gr{k}.
Hence, setting $k=\maxlive$, the interference graph of a code under SSA form is always \gr{k}. %FAB: Might want to make atheorem out of it so as to emphasize it?
...
\begin{description}
  ...
  \item[George] merges~$u$ and $v$ if all neighbors of $u$ with 
    ...
    the original graph, thus \gr{k} too.
\end{description}
...
Originally, those rules were used for any graph, not necessarily \gr{k}, and with an additional clique of pre-colored nodes---the physical machine registers.
...
But in the context of \gr{k} graphs, no restrictions need to be applied.
...
The advocated scheme which corresponds to the pseudo-code of Figure~\ref{fig:ra:code:IRC} and depicted in Figure~\ref{fig:ra:brute}, tries the coalescing of a given affinity only once, and thus does not require any complex freezing mechanism as done in the original IRC.
This is made possible thanks to the following enhancement of the conservative coalescing rule:
Recall that the objective of Briggs and George rules is to test whether the coalescing $v$ and $u$ breaks the \gr{k} property of $G$ or not.
...
On the other hand, the linear-scan extension proposed by Barik in his thesis~\cite{barik2010efficient}, is an interesting competitor, its footprint being, in theory, more compact than the one used by a tree-scan.
...
FAB: ICI. RELIRE section avant.

\paragraph{Handling Registers Constraints}
...
\cite{linear-pfeifer,sarkar2007extended}
PARLER DE: clique de registres, anthipaties de Colombet et al.~\cite{ColombetOct11}, extension de tree-scan de colombet et al.

\paragraph{Handling Vector Registers}
PARLER DE: puzzle solver~\cite{Pereira:2008:PLDI}, Generalized graph coloring~\cite{Smith04} and extension~\cite{Tavares}

\paragraph{Handling Addressing Mode with non-Memory Operands}
PARLER DE: build again dans l'itérated; papier CASES de bouchez sur le spill
...
\paragraph{Coalescing Optimization Problems}
