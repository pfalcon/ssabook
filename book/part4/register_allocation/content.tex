...
\paragraph{Linear-scan} The linear-scan principle is to consider that 
a procedure as a long basic block (\todo{where lr are intervals}), and to scan 
...
\todo{When we encounter the end of its live-range (e.g., , we free the register 
it was assigned to.)}
...
\begin{figure}
  \todo{ caption linearize: interval = sur-approx de liverange réel.}
  \todo{ add variable p def au début et use à la fin}
  \todo{ référencer la figure plus tôt que juste 'comparaison'}

  \includegraphics[height=\textheight]{figures/example-initial}

  \caption{Example}

  \label{fig:ra:old-running}
\end{figure}

\begin{figure}
  \begin{center}
    \subfloat[Linear scan]{\tikzfigure{linear-scan}}
...
\begin{figure}
  \begin{center}
    \subfloat[Splitting variable $x$]{\tikzsubfigure[2]{running}}
...
Let us look at the example Figure~\ref{fig:ra:running}, that presents a very 
...
different times.  For instance, in Figure~\ref{fig:ra:running} (\todo{sub}), we 
can split $x$ in the right branch by changing the definition to $x'$ and using 
a copy $x \gets x'$ at the end of the block:
...
\begin{figure}[htbp]
  \todo{ update figure for running example: from previous part or from part on scan spill }
	\begin{center}
