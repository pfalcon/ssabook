...
Its objective is to establish $\maxlive\le\regs$ (\maxlive lowering) by inserting loads and stores into the program. 
...
Whenever the register pressure is too high, the spilled variable is the one whose next use is the furthest away; 
...
\begin{verbatim}
Further_First(B)
  function spill(p, &in_regs, &in_mem, &to_store, in_solidstate)
    while |in_regs|>R
      let v in in_regs - in_solidstate with further next_use_after(p)
      if v not in in_mem 
         to_store = to_store U {v}
         in_mem = in_mem U {v}
      in_regs = in_regs \ {v}

  foreach prog point p in B from top to bottom
    var_to_store = var_to_load = emptyset
    foreach v used at p
       if v not in var_in_regs 
          var_to_load = var_to_load U {v}
          var_in_regs = var_in_regs U {v}
    spill(p, var_in_regs, var_in_mem, var_to_store, var_to_load)
    foreach v last use at p  
      var_in_regs = var_in_regs \ {v}
    foreach v defined at p
      var_in_regs = var_in_regs U {v}
    spill(p, var_in_regs, var_in_mem, var_to_store, {v defined at p})
    foreach v in var_to_load
      insert a load of v from memory just before p
    foreach v in var_to_store
