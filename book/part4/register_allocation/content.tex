...
We have two candidates for spilling: $x$ and $y$, and the classical furthest first criteria would be different depending on the chosen branch.

\begin{itemize}
  \item If the left branch is taken, considering an execution trace ($AB^{100}C$):
  ...
  It is more profitable to evict variable $x$ (at distance 101).

  % Looking at this execution trace in isolation, and using the further first strategy described above for straight-line code

\item If the right branch is taken, the execution trace is ($AD$).
  In that case, this is variable $y$ has the further use (at distance 2) so we would evict variable $y$.

\end{itemize}

So we have two opposing viewpoints, but looking at the example as a whole, we see that the left branch is not under pressure, so spilling $x$ would only help for program point $p_0$, and one would need to spill another variable in block $D$ ($x$ is used at the beginning of $D$), hence it would be preferable to evict variable $y$.

Still, on the other hand, if we modify a little bit the example by assuming a high register pressure within the loop at program point $p_1$ (by introducing other variables), then evicting variable $x$ would be preferred in order to avoid a load and store in a loop!
...
We will then replace the notion of ``distance'' in the furthest first algorithm with a notion of ``profitability,'' i.e., a measure of the number of program points (weighted by frequency) that would benefit from the spilling of a variable $v$.
...
\begin{figure}
  \begin{center}
    \subfloat[\label{sub:ra:loopentry_example_a}$x \in \vir$ at $B$'s entry]{
    % \qquad\includegraphics[width=0.15\textwidth]{figures/loopentry_example_a.pdf}\qquad
      \tikzfigure{loop-entry-example-a}
    }
    \hfil
    \subfloat[\label{sub:ra:loopentry_example_b}$x \notin \vir$ at $B$'s entry]{
    % \qquad\quad\includegraphics[width=0.15\textwidth]{figures/loopentry_example_b.pdf}
      \tikzfigure{loop-entry-example-b}
  \qquad\quad}
  \end{center}
  \caption{\label{fig:ra:loopentry_example}Initial values of $B.\vir$ at loop entry. Illustrating examples.}
\end{figure}

For a basic block at the entry of a loop, as illustrated by the example of Figure~\ref{fig:ra:loopentry_example}, one do not want to account for allocation on the predecessor basic block, but start from scratch instead.
...
\begin{enumerate}
  \item Example~\subref{sub:ra:loopentry_example_a}: 
    even if at the end of the predecessor basic block, at $p_0$, $x$ is not available in a register, one wants to insert a reload of $x$ at $p_1$, i.e., include $x$ in $B.\vir$.
  Not doing so would involve a reload at every iteration of the loop at $p_2$. 
\item Example~\subref{sub:ra:loopentry_example_b}: 
  Even if at the entry of the loop, $x$ is available in a register, one wants to spill it at $p_1$ and restore at $p_4$ so as to lower the register pressure that is too high within the loop. Not doing so would involve a store in the loop at $p_2$ and a reload on the back edge of the loop at $p_3$.
...
If the Briggs and George tests fail, Testing this property can be done by running function \emph{Simplify(G)} itself!
