...
\begin{algorithm}
  ...
  \Fn{assign\_color ($p$, available)}{
    ...
    \ForEach{$v$ last use at $p$}{
      available[color($v$)] $\gets$ \KwTrue\tcc*{colors not used anymore}
    }
    \ForEach{$v$ defined at $p$}{
      $c \gets$ choose\_color($v$, available)\tcc*{pick one of the available colors}
...
A pruned version of the coalescing algorithm used in the IRC (we removed the freezing mechanism mentioned below and dropped of the spilling part), is depicted in Figure~\ref{fig:ra:code:IRC}.
In this code, coalescing and simplification (that builds the stack) are combined:
...
while V != {} do
  if simplifiable is non empty, let v in simplifiable
    ...
    let a=(v,w) in A  of G of highest weight
    if a in V: A = A \ {a}
    elsif can_be_coalesced(a)
      merge v and w into vw
...
The idea is to simply enhance the conservative coalescing rule (\verb+can_be_coalesced+ of pseudo-code \verb+Simplify_and_Coalesce+) as follow:

%FAB JE SUIS ICI

The idea is pretty simple:
Once an affinity is selected for coalescing, Briggs rule is tested first.
If it succeeds, the two nodes are coalesced.
If not, George rule is tested
