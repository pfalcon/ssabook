...
Wherever it is alive, storage needs to be allocated for that variable; 
ideally a register. 
The set of all program points where a variable is alive is called the \emph{live-range} of the variable. 
The resource conflict of two variables is called \emph{interference} and is usually defined via liveness: 
Two variables interfere if (and only if) there exists a program point where they are simultaneously alive.\footnote{ This definition of interference by liveness is an over-approximation (see Section~\ref{sec:properties_and_flavors:ultimate_interference}). 
  There are refined definitions that create less interferences (see Chapter~\ref{chapter:alternative_ssa_destruction_algorithm}). 
...
One can observe that \maxlive expresses the minimum required number of available registers for a spill-free register assignment. 
Interestingly, for a straight-line code (basic-block), \maxlive constitutes also a sufficient number of registers. 
As one can illustrate through the example of Figure~\ref{fig:ra:linearscan}, a greedy assignment considering live intervals from top to bottom always succeeds with \maxlive registers (equal to two in this example). 
Assume one have only two registers available (denoted $R1$ and $R2$): 
variable $a$ can be assigned $R1$; 
$d$ must then be assigned $R2$; 
at definition point of $e$, $R1$ is released (end of live-range of $a$), so $e$ can be assigned to $R1$...
Note that, the order along which live-ranges are assigned to registers is important.
As an example, by not following the program order e.g. $(a,e,d)$ (as opposed to ($a,d,e$)), a greedy assignment could fail (e.g. if $a$ is assigned to $R1$, $e$ to $R2$).
That important observation gave rise to the linear scan register allocation scheme widely adopted by just-in-time compilers.
...
However, for a general control-flow graph, classical register allocation schemes like to think of interferences as an undirected \emph{interference graph:} The nodes are the variables of the program, and two nodes are connected if they interfere.
...
Unfortunately graph $k$-coloring is known to be NP-complete problem, and for any arbitrary undirected graph, there is a program whose interference graph is this graph.
...
The somewhat non intuitive result is that, even if at every program point there are no more than \maxlive variables alive, we still might need more than $\maxlive$ registers for a correct register allocation!
...
Of course, such an extreme live-range splitting introduces \emph{a lot} of shuffle code, which degrades the run-time of the compiled program. 
...
Whatever the coloring heuristic that is used, live-range splitting (up to the extreme setting of splitting at every program point) will always allow to overcome over spilling decisions.
But if one wants to avoid spill code (memory loads and stores) as much as possible, too many register-to-register copies should be avoided also.
...
Thus, the splitting or not splitting trade-off that has to be addressed by the register allocator.
SSA form based live-range splitting is an elegant solution to this trade-off problem as it is sufficient in bringing the interference graph \maxlive-colorable without blowing-up the number of live-ranges.
...
it always succeeds to color the tree-shaped live-ranges with \maxlive colors. 
...
The direct consequence is that, as opposed to general form programs, the only case where spilling is required is here when $\maxlive > k$. 
...
In practice, a post-pass of a graph coloring scheme scans each basic-block separately, so as to, whenever possible, allow multiple uses of a spilled variable to reuse the same reload.
This corresponds to keep parts of the live-range of a spilled variable in a register.
Linear scan allocators, do natively, perform such local optimization.
...
In both cases, we will assume that the program was in SSA before spilling, and consider the spilling phase as an SSA program transformation that establishes $\maxlive\le\regs$ (\maxlive lowering) by inserting loads and stores into the program.
...
1. the constraints that describe the universe of possible solutions expresses that the resulting code should be \regs-colorable;
2. the objective function expresses the fact that the (weighted) amount of inserted loads and stores should be minimized.
...
As for the objective, it expresses itself with the following profitability test: 
...
The other (related) implication of the use of SSA form follows from the following observation: 
...
\subsection{Finding Spilling Candidates: Graph versus Scan}
...
The interference graph is built through a simple traversal of the CFG: To an interference-graph node $p$ is associated an SSA program variable $p.\textit{var}$ and vis-versa. There is an interference edge from $v$ to $p$ if and only if $v.\textit{var}$ is alive at $p.\textit{var}$ definition point. Observe that, as opposed to standard interference graphs, we consider directed edges here. Also, to each node $p$, we associate a field $p.\textit{pressure}$ that corresponds to the number of variable alive at definition point of $p.\textit{var}$. As the edge $(v,p)$ is added to the graph, a boolean $(v,p).\textit{high}$ is set with value true if and only if $p.\textit{pressure}>\regs$. Another field $v.\textit{useful}$, that is attached to every node $v$ is also updated: if $p.\textit{pressure}>\regs$ then $p.\textit{useful}$ is set to 1 (otherwise set to 0), and for all $(v,p)$, $p.\textit{pressure}$ gets incremented by one. At the end of the build process, $v.\textit{useful}$ expresses the number of program points that belong to the live-range of $v.\textit{var}$ for which the register pressure is greater than \regs. In other words,
$$v.\textit{useful}=\left|\lbrace p,\ \left(\exists (v,p) \wedge (v,p).\textit{pressure}>\regs\right) \vee \left( v=p \wedge p.\textit{pressure}>\regs\right) \rbrace\right|$$
...
Spilling $v$ is useful as it would reduce the size of the clique by one.

Those additional fields of the interference graph can be updated as variables/nodes are spilled. 
When a node $n$ is spilled:
1.~for all its incoming edges $(v,n)$, $v.\textit{useful}$ is decremented by one;
...
a spill everywhere will place a superfluous (and costly!) load in that loop.
...
In the context of a basic block, a simple algorithm that works well is the ``furthest first'' algorithm.
The idea is to scan the block from to bottom,
whenever where the register pressure is too high, we spill the variable whose next use is the furthest away;
And we spill it only \emph{up to this next use}.
...
We will present in the next section an algorithm that extends this ``furthest first'' algorithm to general control-flow graphs.
The idea is to use a data-flow analysis to approximately compute the next use values on the control-flow graph.
In this next use calculation, loops are virtually unrolled by a large factor such that code ``behind'' loops appears far away from code in front of the loop.
This forces the spilling algorithm to prefer variables that are not used in loops and to leave in registers the parts of live-ranges that are used inside loops.
We then have to find how and in which order to process the basic blocks of the control-flow graph in order to determine a reasonable initial register occupation for each block.
