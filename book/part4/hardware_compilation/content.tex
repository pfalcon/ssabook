...
for a single clock cycles and stores the outputs of the operations in the
output registers. 
...
program variable leads to the use of a $\phi$-function.
This $\phi$-function abstraction thus corresponds in terms
...
When targeting a hardware platform, that advantage of the SSA
...
This example highlights the potential benefits of a SSA-based, when
...
each of the assumed values of the variable. Using separate registers 
in hardware registers for disjoint live ranges, allows hardware generation
...
the mapping to architectures such as VLIW~\cite{CompilingVLIWBook}
or Systolic Arrays~\cite{CompilingSystolicArraysBook}. 
...
The generation of the hardware circuit simply uses the register's 
value defined in the representation an intermediate value
corresponding to the name of each of the variable referenced
in the predicate expression of this Gated-SSA form\footnote{As opposed
...
into the multiplexers associated with each $\phi$-function; a) source code; b)
...
values. For the examples in Figures~\ref{fig:Fig.4.8}(a,b) the compiler
would quickly understand the common variables {\tt a} in the two 
expressions associated with the $\phi$-function for the {\tt y} 
variable.\\

A second transformation that can be applied to multiplexers is 
specific FPGA whose basic building block consists of a k-input
